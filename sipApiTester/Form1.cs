...
namespace sipApiTester
{
    ...
    string strHtml)
        ...
        {
            var ret = FormClientInfo.Initial("sipApiTester",
() => StringUtil.IsDevelopMode());
        ...
        private void MenuItem_test_connect_Click(object sender, EventArgs e)
        {
            Task.Run(async () =>
            {
                try
                {
                    var result = await TestConnect.TestConnect1(1000);
                    if (result.Value == -1)
        ...
        private void MenuItem_file_clearHtmlText_Click(object sender, EventArgs e)
        {
            this.Invoke((Action)(() =>
            {
                ClearHtml(this.webBrowser1);
        ...
        private void MenuItem_test_loginSequence_Click(object sender, EventArgs e)
        {
            Task.Run(async () =>
            {
                try
                {
                    var result = await TestConnect.TestConnectAndLogin(300);
                    if (result.Value == -1)
                        DataModel.SetMessage(result.ErrorInfo);
                }
                catch (Exception ex)
        ...
        private void MenuItem_test_loginConcurrent_Click(object sender, EventArgs e)
        {
            Task.Run(async () =>
            {
                try
