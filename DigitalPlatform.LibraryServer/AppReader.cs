...
namespace DigitalPlatform.LibraryServer
{
    ...
    strTextNew);
            ...
            {
                strError = "strAction值必须为change、changestate和changeforegift之一。";
                return -1;
            }
        ...
        // paramters:
        //      strAction    操作。new change delete changestate changeforegift forcenew forcechange forcedelete
        ...
        //      特殊操作可能还需要 changereaderstate 和 changereaderforegift 权限
        // 日志:
        //      要产生日志
            ...
            out DigitalPlatform.rms.Client.rmsws_localhost.ErrorCodeValue kernel_errorcode)
        {
                ...
                || strAction == "forcedelete")
            {
                ...
                {
                    result.Value = -1;
                    result.ErrorInfo = "修改读者信息的" + strAction + "操作被拒绝。不具备restore权限。";
            ...
            if (strAction == "delete")
            {
                ...
                {
                    strError = "strAction值为delete时, strNewXml参数必须为空";
                    goto ERROR1;
                }
                if (baNewTimestamp != null)
                {
                    strError = "strAction值为delete时, baNewTimestamp参数必须为空";
                    goto ERROR1;
                }
            }
            else
            {
                // 非delete情况 strNewXml则必须不为空
                if (String.IsNullOrEmpty(strNewXml) == true)
                {
                    strError = "strAction值为" + strAction + "时, strNewXml参数不能为空";
                    goto ERROR1;
                }
            }
            ...
            if (strAction == "new")
            {
                ...
                {
                    strError = "strAction值为new时, strOldXml参数必须为空";
                    goto ERROR1;
                }
                if (baOldTimestamp != null)
                {
                    strError = "strAction值为new时, baOldTimestamp参数必须为空";
                    goto ERROR1;
                }
            }
            else
            ...
            {
                strError = "strOldXml XML记录装载到DOM时出错: " + ex.Message;
                goto ERROR1;
            }
            ...
            {
                strError = "strNewXml XML记录装载到DOM时出错: " + ex.Message;
                goto ERROR1;
            }
           ...
           // 对读者身份的附加判断
            ...
            else if (strAction == "delete")
            {
                ...
                {
                    strError = "没有必要在delete操作的strNewXml, 包含新记录内容...。相反，注意一定要在strOldXMl中包含即将删除的原记录";
                    goto ERROR1;
                }
            ...
            if (bExist == true) // 2008/5/29 new add
            {
                ...
                if (nRet == 1)  // 读者证条码号有改变
                {
                        ...
                        && bForce == false) // 2008/5/28 new add
                    {
                        ...
                        strError = "因读者记录 '" + strRecPath + "' 中包含有 " + strDetailInfo + "，所以修改它时证条码号字段内容不能改变。(当前证条码号 '" + strOldBarcode + "'，试图修改为条码号 '" + strNewBarcode + "')";
                        goto ERROR1;
                    }
                }
