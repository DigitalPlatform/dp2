...
namespace DigitalPlatform.LibraryServer
{
    ...
    public partial class LibraryApplication
    {
        ...
        REDO_WHOLE:
            ...
            {
                var parts = StringUtil.ParseTwoPart(strItemKey, ".");
                strItemKey = parts[1];
                strOwnerInstitution = parts[0];
            }
            */
            ParseOI(strItemBarcodeParam, out string strItemKey, out string strOwnerInstitution);
            // 2021/3/9
            ParseOI(strReaderBarcodeParam, out string strReaderKey, out string strPatronOI);
            ...
            if (bRenew == false)
            {
                ...
                if (sessioninfo.UserType == "reader"
                    && sessioninfo.Account != null
                    // && strReaderKey != sessioninfo.Account.Barcode 
                    && dp2StringUtil.MatchReaderKey(strReaderKey, sessioninfo.Account.Barcode, sessioninfo.Account.PatronRefID) == false
                    && string.IsNullOrEmpty(strPersonalLibrary) == true)
                {
                    result.Value = -1;
                    result.ErrorInfo = $"借阅操作被拒绝。{sessioninfo.Account.GetReaderDisplayKey()} 作为读者不能代他人进行借阅操作。";
                    result.ErrorCode = ErrorCode.AccessDenied;
                    return result;
                }
            }
            ...
            if (string.IsNullOrEmpty(strReaderKey) == false)
            {
                ...
                //      0       所给出的字符串不是读者证号二维码
                //      1       成功      
                nRet = DecodeQrCode(strReaderKey,
                ...
                if (nRet == 1)
                {
                    strQrCode = strReaderKey;
                    strReaderKey = strOutputCode;
                }
            }

            // 因为 strReaderKey 可能是 PQR:xxxx 形态，需要替换为真正的证条码号以后才能参与判断，所以放到这里判断
            if (bRenew == true)
            {
                // 对读者身份的附加判断
                // 注：具有个人书斋的，还可以继续向后执行
                    ...
                    && string.IsNullOrEmpty(strPersonalLibrary) == true)
                {
                    ...
                    // text-level: 用户提示
                    // result.ErrorInfo = this.GetString("续借操作被拒绝。作为读者不能代他人进行续借操作。");  // "续借操作被拒绝。作为读者不能代他人进行续借操作。"
                    result.ErrorInfo = string.Format("续借操作被拒绝。作为读者 '{0}' 不能代他人 '{1}' 进行续借操作。", sessioninfo.Account.GetReaderDisplayKey(), strReaderKey);  // TODO: 测试验证完成后需要改回用上一句
                    result.ErrorCode = ErrorCode.AccessDenied;
                    return result;
                }
            }
            ...
            string strLockReaderBarcode = strReaderKey; // 加锁专用字符串，不怕后面被修改了
        ...
        REDO_BORROW:
            ...
            try // 读者记录锁定范围开始
            {
                ...
                {
                    bool bReaderDbInCirculation = true;
                    //      strReaderKey    [in,out] 读者证条码号或者 @refID:xxx 形态。函数中可能会把它清空
                ...
                out string strHitFrom);
                    ...
                    if (bReaderDbInCirculation == false)
                    {
                        ...
                        // "借书操作被拒绝。读者证条码号 '" + strReaderKey + "' 所在的读者记录 '" +strOutputReaderRecPath + "' 因其数据库 '" +strReaderDbName+ "' 属于未参与流通的读者库";
                        goto ERROR1;
                    }
                    ...
                    if (String.IsNullOrEmpty(strIdcardNumber) == false
                        || string.IsNullOrEmpty(strReaderKey) == true /* 2013/5/23 */)
                    {
                        // 获得读者证条码号
                        strReaderKey = DomUtil.GetElementText(readerdom.DocumentElement,
                            "barcode");
                    }
                    ...
                    if (nRet == 1)
                    {
                        // strError = "当前用户 '" + sessioninfo.UserID + "' 的存取权限或好友关系禁止操作读者(证条码号为 " + strReaderKey + ")。具体原因：" + strError;
                        result.Value = -1;
                ...
                // 加册记录锁
                this.EntityLocks.LockForWrite(strItemKey == null ? "" : strItemKey);
    ...
    out item_timestamp);
                    ...
                    if (aPath.Count > 1)
                    {
                        ...
                        // "册条码号为 '" + strItemKey + "' 的册记录有 " + aPath.Count.ToString() + " 条，无法进行借阅操作。请在附加册记录路径后重新提交借阅操作。";
                        result.ErrorCode = ErrorCode.ItemBarcodeDup;
                    ...
                    if (String.IsNullOrEmpty(strOutputItemRecPath) == false)
                    {
                        ...
                        {
                            // text-level: 内部错误
                            strError = "借书操作被拒绝。册条码号 '" + strItemKey + "' 所在的册记录 '" + strOutputItemRecPath + "' 因其数据库 '" + strItemDbName + "' 属于未参与流通的实体库";
                            goto ERROR1;
                        }
                    }
    ...
    "parent");
                    ...
                    if (string.IsNullOrEmpty(strReaderKey) == true)
                    {
                        ...
                        {
                            strError = "册 '" + strItemKey + "' 当前未曾被任何读者借阅，所以无法进行" + strActionName + "操作";
                            goto ERROR1;
                        }
#if NO
                    // 如果提供了读者证条码号，则需要核实
                    if (String.IsNullOrEmpty(strReaderBarcode) == false)
                    {
                        ...
                        if (String.IsNullOrEmpty(strReaderKey) == true)
                            strReaderKey = strOutputReaderBarcode;

                        // *** 如果读者记录在前面没有锁定, 在这里锁定
                        if (bReaderLocked == false)
                        {
                            Debug.Assert(string.IsNullOrEmpty(strReaderKey) == false, "");
                            // 2015/9/2
                            nRedoCount++;
                            if (nRedoCount > 10)
                            {
    ...
    "barcode");
                ...
                out strError);
                    ...
                    {
                        domOperLog.LoadXml("<history_container />");
                        ...
                        if (lRet == -1) // 初次Undo失败
                        {
                            if (channel.ErrorCode == ChannelErrorCode.TimestampMismatch)
                            {
                                ...
                                nRet = UndoBorrowReaderRecord(
                                    ...
                                    strOutputReaderRecPath,
                                    // strReaderKey,
                                    DomUtil.GetElementText(readerdom.DocumentElement,
                                    "refID"),
                                    DomUtil.GetElementText(itemdom.DocumentElement,
                                    "barcode"),
                                    DomUtil.GetElementText(itemdom.DocumentElement,
                                    "refID"),
                                    out strError);
                                ...
                                {
                                    // text-level: 内部错误
                                    strError = $"Borrow({api_summary}) Undo读者记录 '" + strOutputReaderRecPath + "' (读者证条码号为'" + strReaderKey + "') 借阅册条码号 '" + strItemKey + "' 的修改时，发生错误，无法Undo: " + strError;
                                    this.WriteErrorLog(strError);
                                    goto ERROR1;
                                }
        ...
        "idcardNumber", strIdcardNumber);
...
#endif
                    if (this.Garden != null)
                        this.Garden.Activate(strReaderKey,
                            strReaderLibraryCode);
                ...
                {
                    // 解册记录锁
                    this.EntityLocks.UnlockForWrite(strItemKey == null ? "" : strItemKey);    // strItemKey 在整个函数中不允许被修改
                }
...
out strError);
    ...
    out strError);
        ...
        static string ClipReaderXml(XmlDocument source, out bool bClipped)
        {
            ...
            ClipElement(target.DocumentElement, "palmprint");
            return target.OuterXml;
        }
            ...
            out string strError)
        {
            ...
            if (rfid == null)
            {
                ...
                */
                ...
                // strError = $"library.xml 中没有配置 rfid 元素，册记录所属机构代码应为空。(但现在是 '{strOwnerInstitution}')";
                strError = $"本册来自馆外机构 '{strOwnerInstitution}'";
                return 0;
            }
            ...
            {
                strError = $"装载册记录 {strOutputItemRecPath} 的 XML 到 DOM 出现异常: {ex.Message}";
                return -1;
            }
...
out strError);
            ...
            out string strError)
        {
            ...
            string strReaderRefID = DomUtil.GetElementText(reader_dom.DocumentElement, "refID");
            ...
            if (sessioninfo.UserType == "reader")
            {
                ...
                if (sessioninfo.UserID == strReaderBarcode
                    || sessioninfo.UserID == $"@refID:{strReaderRefID}")
                    return 0;

                // TODO: 好友列表里面是列出证条码号，还是可以用 @refID:xxx 形态？
                ...
                if (string.IsNullOrEmpty(strFields) == false)
                {
                    ...
                    nRet = this.ConvertReaderBarcodeListToRefIdList(channel,
    strFields,
    out string strReaderRefIdList,
    out strError);
                    if (nRet == -1)
                    {
                        strError = $"在翻译好友列表为读者参考 ID 的过程中出错: {strError}";
                        return -1;
                    }
                    // 判断当前用户是否为 reader_dom 读者的 friends
                    if (StringUtil.IsInList($"@refID:{sessioninfo.Account.PatronRefID}", strReaderRefIdList) == true)
                        return 0;
                    strError = "'" + sessioninfo.Account.GetReaderDisplayKey() + "' 不在 '" + strReaderBarcode + "' 的好友列表中";
        ...
        out string strRoom);
            ...
            if (nRet == -2)
            {
                    ...
                    || location.CanBorrow == "no")
                {
                    ...
                    // "册 " + strItemKey + " 的馆藏地点为 " + strLocation + "，按规定(<locationTypes>配置)此册不允许外借。";
        ...
        message);
            ...
            else
            {
                ...
                if (bResultValue == false)
                {
                    ...
                    strError = "不允许" 
                        + (bRenew == true ? "续借" : "外借")
                        + "。因为册 " + strItemKey + " 的状态为 "+strMessageText;
                     * */
                    return 0;
                }
            }
    ...
    "refID");
            ...
            if (String.IsNullOrEmpty(strItemBarcode) == true)
            {
                ...
                strItemBarcodeParam = "@refID:" + strItemRefID;
            }
#endif
            // 2024/4/12
            if (String.IsNullOrEmpty(strItemBarcode) == true
    ...
    .SetValue("itemRecord", new ItemRecord(itemdom));
        ...
        message);
            ...
            if (string.IsNullOrEmpty(strPersonalLibrary) == false)
            {
                    ...
                    && StringUtil.IsInList(strRoom, strPersonalLibrary) == false && StringUtil.IsInList(strRoomOrigin, strPersonalLibrary) == false)
                {
                    if (strRoom != strRoomOrigin)
                        strError = "当前用户 '" + account.GetReaderDisplayKey() + "' 只能操作馆代码 '" + strReaderLibraryCode + "' 中地点为 '" + strPersonalLibrary + "' 的图书，不能操作地点为 '" + strRoom + "' (和 '" + strRoomOrigin + "')的图书";
                    else
                        strError = "当前用户 '" + account.GetReaderDisplayKey() + "' 只能操作馆代码 '" + strReaderLibraryCode + "' 中地点为 '" + strPersonalLibrary + "' 的图书，不能操作地点为 '" + strRoom + "' 的图书";
            ...
            if (bRenew == false)
            {
                ...
                var node = LookupBorrowElement(readerdom,
                    strItemRefID,
                    strItemBarcode);
                if (node != null)
                {
...
out _);
        ...
        //      strReaderKey    读者证条码号。若需要检查记录，看看里面条码号是否已经变化了，就使用这个参数。如果不想检查，就用null
        //      strItemKey  已经借的册条码号
        // return:
        //      -1  error
        //      0   没有必要Undo
    ...
    out strError);
            ...
            {
                string strCurrentReaderRefID = DomUtil.GetElementText(readerdom.DocumentElement,
                    "refID");
                if (strReaderRefID != strCurrentReaderRefID)
                {
                    strError = "发现从数据库中读出的读者记录 '" + strReaderRecPath + "' ，其 refID 字段内容 '" + strCurrentReaderRefID + "' 和要Undo的读者记录参考ID '" + strReaderRefID + "' 已不同。";
                    return -1;
                }
            }

            // 去除dom中表示借阅的节点
            // XmlNode node = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@barcode='" + strItemKey + "']");
            var node = LookupBorrowElement(readerdom,
                strItemRefID,
                strItemBarcode);
            if (node == null)
            ...
            out string strOwnerInstitution)
        {
            ...
            {
                var parts = StringUtil.ParseTwoPart(strItemBarcodeParam, ".");
                strItemBarcode = parts[1];
                strOwnerInstitution = parts[0];
            }
        }
            ...
            out ReturnInfo return_info)
        {
            ...
            */
            ...
            string strReservationReaderKey = "";    // 预约者检索 key。可能是 "@refID:" 形态
            string strReservationReaderDisplayBarcode = ""; // 用于显示的预约者证条码号
            string strNotifyID = "";
    ...
    out item_timestamp);
                    ...
                    if (aPath != null && aPath.Count > 1 && strFrom == "册条码号")
                    {
                        ...
                        if (String.IsNullOrEmpty(strPureReaderBarcode) == true)
                        {
                            ...
                            // 如果没有给出读者证条码号参数
                            result.Value = -1;
                            result.ErrorInfo = "册条码号为 '" + strItemBarcode + "' 册记录有 " + aPath?.Count.ToString() + " 条，无法进行还书操作。请在附加册记录路径后重新提交还书操作。";
                            result.ErrorCode = ErrorCode.ItemBarcodeDup;
                        ...
                        {
                            result.Value = -1;
                            result.ErrorInfo = "册条码号 '" + strItemBarcode + "' 检索出的 " + aPath?.Count + " 条记录中，没有任何一条其<borrower>元素表明了被读者 '" + strReaderBarcode + "' 借阅。";
                            result.ErrorCode = ErrorCode.ItemBarcodeNotFound;
                            return result;
                        }
                        ...
                        if (nRet > 1)
                        {
                            ...
                            result.Value = -1;
                            result.ErrorInfo = "册条码号为 '" + strItemBarcode + "' 并且<borrower>元素表明为读者 '" + strReaderBarcode + "' 借阅的册记录有 " + aFoundPath.Count.ToString() + " 条，无法进行还书操作。请在附加册记录路径后重新提交还书操作。";
                            result.ErrorCode = ErrorCode.ItemBarcodeDup;
                            this.WriteErrorLog(result.ErrorInfo);   // 2012/12/30
                        ...
                        this.WriteErrorLog("借书遇册条码号 '" + strItemBarcode + "' 重复但根据读者证条码号 '" + strReaderBarcode + "' 成功去重");   // 2012/12/30
            ...
            "circulation");
                    ...
                    if (string.IsNullOrEmpty(strItemXml) == false)
                    {
                        ...
                        {
                            strError = "册记录 '" + strItemBarcode + "' 格式错误: " + strError;
                            goto ERROR1;
                        }
    ...
    out strError);
                        ...
                        if (strAction == "inventory" || strAction == "transfer")
                        {
                            ...
                            if (string.IsNullOrEmpty(strOutputReaderBarcode) == false)
                            {
                                ...
                                strInventoryWarning = "册 " + strItemBarcode + " 当前处于被借阅状态。如确属在架已还图书，建议立即为之补办还书手续。" + " (册记录路径为 '" + strOutputItemRecPath + "')";
                            }
                        }
                        else
                        {
    ...
    "location");
                    ...
                    if (bDelayVerifyReaderBarcode == true)
                    {
                        ...
                        {
                            if (strOutputReaderBarcode != strPureReaderBarcode/*strReaderKey*/
                                && strOutputReaderBarcode != $"@refID:{strReaderRefID}")
                            {
                    ...
                    */
                    ...
                    if (itemdom != null)
                    {
                        ...
                        DomUtil.SetElementText(domOperLog.DocumentElement, "itemBarcode",
                            string.IsNullOrEmpty(strRealItemBarcode) == false ? strRealItemBarcode : strItemBarcode);
                        /* 后面会写入<overdues>
                        if (nRet == 1)
                        {
                            // 如果有超期和或丢失处理信息
        ...
        strBiblioRecPath);
                    ...
                    */
                    ...
                    if (strAction == "read")
                    {
                        string strBiblioRecPath = "";
                        string strVolume = "";
                        if (IsBiblioRecPath(strItemBarcode) == false)
                        {
                            strBiblioRecID = DomUtil.GetElementText(itemdom.DocumentElement, "parent"); //
                            ...
                            strBiblioRecPath = strItemBarcode.Substring("@biblioRecPath:".Length);

                        // 探测 mongodb 库中是否已经存在这样的事项
                        // TODO: 注意检查 strReaderBarcode 内容，看看是否符合(条码/参考ID)改造的要求
                        IEnumerable<ChargingOperItem> collection = this.ChargingOperDatabase.Exists(
                            "@refID:" + DomUtil.GetElementText(readerdom.DocumentElement,
                            "refID"),    // strReaderBarcode,
                            "", // string.IsNullOrEmpty(strItemKey) == false ? strItemKey : strItemKey,
                            strBiblioRecPath,
                            strVolume,
                    ...
                    if (strAction == "boxing")
                    {
                        ...
                        // parameters:
                        //      strReaderKey    读者证条码号或者 "@refID:xxxxx"
                        //      strItemKey  册条码号或者 "@refID:xxxxx" 
                    ...
                    if (strAction != "boxing")
                    {
                        ...
                        // parameters:
                        //      strReservationReaderKey [out] 返回值可能是 证条码号，也可能是 @refID:xxx 形态
                        // return:
                        //      -1  error
                        //      0   没有修改
                        //      1   进行过修改
                        nRet = DoItemReturnReservationCheck(
                            ...
                            ref itemdom,
                            out strReservationReaderKey,
                            out strNotifyID,
                            out strError);
                        if (nRet == -1)
                            goto ERROR1;

                        strReservationReaderDisplayBarcode = strReservationReaderKey;
        ...
        "idcardNumber", strIdcardNumber);
                    ...
                    if (bItemDbInCirculation == false)
                    {
                        if (String.IsNullOrEmpty(result.ErrorInfo) == false)
                            result.ErrorInfo += "\r\n";
                        result.ErrorInfo += "册条码号 '" + strItemBarcode + "' 所在的册记录 '" + strOutputItemRecPath + "' 其数据库 '" + StringUtil.GetDbName(strOutputReaderRecPath) + "' 属于未参与流通的实体库。";
                        result.Value = 1;
                    }
                    ...
                    if (String.IsNullOrEmpty(strReservationReaderKey) == false // 2009/10/19 changed  //bFoundReservation == true
                        && strAction != "lost" && strAction != "boxing")
                    {
                        // 为了提示信息中出现读者姓名，这里特意获取读者姓名
                        string strReservationReaderName = "";

                        if (strReaderBarcode == strReservationReaderKey
                            || "@refID:" + strReaderRefID == strReservationReaderKey)
                        ...
                        else
                        {
                            ...
                            //      0   not found
                            //      1   found
                            nRet = GetReaderNameAndBarcode(
                                sessioninfo,
                                strReservationReaderKey,
                                out strReservationReaderName,
                                out string temp,
                                out strError);
                            if (string.IsNullOrEmpty(temp) == false)
                                strReservationReaderDisplayBarcode = temp;
                        ...
                        if (String.IsNullOrEmpty(result.ErrorInfo) == false)
                            result.ErrorInfo += "\r\n";
                        result.ErrorInfo += "因本册图书已被读者 " + strReservationReaderDisplayBarcode/*strReservationReaderKey*/ + " "
            ...
            if (String.IsNullOrEmpty(strReservationReaderKey) == false
                && strAction != "lost" && strAction != "boxing")
            {
                DateTime start_time_1 = DateTime.Now;
    ...
    "Return() 中预约到书通知 耗时 ");
                ...
                result.ErrorInfo += "还书操作成功。因此册图书被读者 " + strReservationReaderKey + " 预约，请放入预约保留架。";
                 * */
    ...
    "Return() 中返回册记录(" + strItemFormatList + ") 耗时 ");
            ...
            */
            ...
            if (StringUtil.IsInList("biblio", strStyle) == true)
            {
                ...
                {
                    if (IsBiblioRecPath(strItemBarcode) == true)
                        strBiblioRecPath = strItemBarcode.Substring("@biblioRecPath:".Length);
...
out string _);
    ...
    out strError);
            ...
            if (String.IsNullOrEmpty(strReaderRefID) == false)
            {
                string strReaderBarcodeContent = DomUtil.GetElementText(readerdom.DocumentElement,
                    "refID");
                if (strReaderRefID != strReaderBarcodeContent)
                {
                    strError = "发现从数据库中读出的读者记录 '" + strReaderRecPath + "' ，其<barcode>字段内容 '" + strReaderBarcodeContent + "' 和要Undo的读者记录证条码号 '" + strReaderRefID + "' 已不同。";
                    return -1;
                }
            }
    ...
    out string strError)
            ...
            out string strError)
        {
            ...
            if (strAction == "overwrite")
            {
                ...
                {
                    /*
                    XmlNode history_container = this.LibraryCfgDom.DocumentElement.SelectSingleNode("calendars");
                    if (history_container == null)
                    {
                        history_container = this.LibraryCfgDom.CreateElement("calendars");
                        this.LibraryCfgDom.DocumentElement.AppendChild(history_container);
                    }
                    */
                    var root = EnsureRoot();
    ...
    out string strError)
        {
            ...
            // 2025/4/25
            // 先检查 strFragment 内容的合法性
            // return:
            //      -1  校验发生错误
            //      0   XML 正确
            //      1   XML 有格式错误
            var ret = VerifyValueTablesXml(strFragment,
                out strError);
            if (ret == -1)
            {
                strError = $"校验 XML 内容合法性时出现错误: {strError}";
                return -1;
            }
            if (ret == 1)
            {
        ...
        //      0   XML 正确
        //      1   XML 有格式错误
        int VerifyValueTablesXml(string strFragment,
            out string strError)
        {
            strError = "";
            var strRootElementName = "valueTables";
            ...
            {
                // strError = "fragment XML装入 XmlDocumentFragment 时出错: " + ex.Message;
                strError = "XML 结构不正确: " + ex.Message;
                return -1;
            }
            ...
            List<XmlElement> library_nodes = new List<XmlElement>();
            library_nodes.Add(source_dom.DocumentElement);
            library_nodes.AddRange(source_dom.DocumentElement.SelectNodes("library").Cast<XmlElement>());

            foreach (var node in library_nodes)
            {
                var table_nodes = node.SelectNodes("table");
                string library_code = node.GetAttribute("code");
                string comment = $"元素 {node.Name}[@code='{library_code}'] 子代的 table 元素";
                if (string.IsNullOrEmpty(library_code))
                    comment = $"元素 {node.Name} 子代的 table 元素";
                var ret = CheckDup(comment, table_nodes, out strError);
                if (ret == -1)
                    return -1;
                if (ret == 1)
                {
        ...
        // 检查 table 元素之间是否发生了 name 或 name + dbname 属性值重复
        // return:
        //      -1  校验发生错误
        //      0   XML 正确
        //      1   XML 有格式错误
            ...
            out string error)
        {
            ...
            {
                var node1 = table_nodes[i] as XmlElement;
                var s1 = GetNameString(node1);
                for (int j = i + 1; j < table_nodes.Count; j++)
                {
                    var node2 = table_nodes[j] as XmlElement;
                    var s2 = GetNameString(node2);
                    if (s1 == s2)
                    {
                        error = $"{comment}之间发生了名字属性重复:\r\n{node1.OuterXml}\r\n{node2.OuterXml}";
                        return 1;
                    }
                }
            }
            ...
            string GetNameString(XmlElement node)
            {
                var name = node.GetAttribute("name");
                var dbname = node.GetAttribute("dbname");
                if (dbname == null)
                    dbname = "";
                return name + "|" + dbname;
            }
    ...
    out string strError)
        ...
        public int GetLibraryFragmentXml(
            string strRootElementName,
            string strLibraryCodeList,
            out string strValue,
            out string strError)
    ...
    out string strError)
        {
            ...
            XmlDocument source_dom = new XmlDocument();
            source_dom.LoadXml("<history_container />");
    ...
    out strError);
        ...
        // 检查每个通知点，返回当前时间已经达到或者超过了那些检查点(时刻)的下标
        // 注1: 如果检查点比 strBorrowDate 时间点更早，或者晚于 strBorrowDate + strPeriod 时间点(也就是应还书点)，则其下标不会被返回
        // 注2: 如果 utc_now 时间点已经超期(也就是晚于应还时间点)，那么 indices 应该返回所有的检查点的下标
        // parameters:
        //      utc_now         检查的当时时间。注意这是 GMT 时间
        //      strBorrowDate   借阅日期。RFC1123 时间格式
        //      strPeriod       借阅期限
        //      strNotifyDef    检查点定义字符串。例如 "-1day,50%"
        //      indices         返回 utc_now 达到或者已经超过了的哪些检查点的下标(从 0 开始计数)
        // return:
        //      -1  数据格式错误
        //      0   成功
    ...
    out strError);
        ...
        //      strCheckPoint   单个的检查点定义。-1day,1hour,-19%,10%
        //              注意，-1day 是指从 end 时刻减去一天(根据范围末端配置检查点)；而 1day 则是指从 start 加上 1 天(根据范围起点配置检查点)
        //              -30% 是指从 end 时刻减去(start 到 end)范围的 30% 长度(根据范围末端配置检查点)；而 30% 是指从 start 时刻加上(start 到 end)范围的 30% 长度(根据范围起点配置检查点)
        // return:
        //      -1  出错
        //      0   不满足
        //      1   满足
    ...
    */
            ...
            out string strError)
        {
...

            {
                ...
                // 获得预约到书队列记录
                // parameters:
                //      strItemKey  册条码号。可以使用 @itemRefID: 前缀
                // return:
                //      -1  error
                //      0   not found
                //      1   命中1条
            ...
            string strItemBarcode)
        {
            ...
            foreach (XmlElement node in nodes)
            {
                ...
                if (StringUtil.IsInList("arrived", state) == false)
                    continue;
                string text1 = node.GetAttribute("arrivedItemBarcode");
                string text2 = node.GetAttribute("arrivedItemRefID");   // 2025/4/23 补充
                if (text2 == strItemBarcode || text1 == strItemBarcode)
                    return node;
        ...
        // 加入超期检查警告
        // parameters:
        //      strItemKey return() API 中的 strItemKey，可能包含 @refID: 前缀部分，还可能是登录号
            ...
            out string strError)
        {
            ...
            if (nodeBorrow == null) // 为了兼容以前的的 barcode 属性
                nodeBorrow = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@barcode='" + strItemKey + "']");
            if (nodeBorrow == null)
            {
                // 再尝试一次直接用 册条码号
                if (string.IsNullOrEmpty(strItemBarcode) == false)
                ...
                if (nodeBorrow == null)
                {
                    strError = "在读者记录表明该读者并未曾借阅过册 '" + strItemKey + "'。";
                    return -1;
                }
            }
             */
            // TODO: strItemBarcode 和 strItemKey 到底是什么内容
            ...
            if (String.IsNullOrEmpty(strOverdueString) != true)
            {
                ...
                // history_container.AppendChild(fragment);
                // 插入到最前面
...
#endif
            // 注: strItemKey 里面为册条码号或者册的参考 ID。注意，优先为册条码号
...
"refID");
            ...
            if (string.IsNullOrEmpty(strPersonalLibrary) == false)
            {
                ...
                {
                    if (strRoom != strRoomOrigin)
                        strError = $"还书失败。当前用户 '{sessioninfo.Account.GetReaderDisplayKey()}' 只能操作馆代码 '{strReaderLibraryCode}' 中地点为 '{strPersonalLibrary}' 的图书，不能操作地点为 '{strRoom}' (和 '{strRoomOrigin}')的图书";
                    else
                        strError = $"还书失败。当前用户 '{sessioninfo.Account.GetReaderDisplayKey()}' 只能操作馆代码 '{strReaderLibraryCode}' 中地点为 '{strPersonalLibrary}' 的图书，不能操作地点为 '{strRoom}' 的图书";
                    return -1;
                }
            }
            ...
            string strBorrower = DomUtil.GetElementText(itemdom.DocumentElement,
                "borrower",
                out XmlNode borrow_node);
            string strBorrowerBarcode = "";
            if (borrow_node != null)
                strBorrowerBarcode = (borrow_node as XmlElement).GetAttribute("barcode");
    ...
    out strError);
            ...
            if (lDelta1 > 0)
            {
                ...
                // 注: '超期违约金因子' 参数值如果为空，表明不对超期做违约金处理(方法是在 strOverdueString 第一字符添加一个 '!' )。但需要记入日志
                {
                    ...
                    /*
                    strOverdueString = "<overdue barcode='" + strItemKey
...
out strError);
            ...
            CONTINUE_LOSTING:
                ...
                string strBorrowRefID = "";
                if (strBorrower != null && strBorrower.StartsWith("@refID:"))
                    strBorrowRefID = "(" + strBorrower + ")";

                string displayBorrower = strBorrower;
                if (string.IsNullOrEmpty(strBorrowerBarcode))
                {
                    var channel = this.GetRmsChannel(sessioninfo);
                    ...
                    if (ret != -1 && string.IsNullOrEmpty(temp) == false)
                        displayBorrower = temp;
                }
                else
                    displayBorrower = strBorrowerBarcode;

                strLostComment = $"本册于 {DateTimeUtil.Rfc1123DateTimeStringEx(now.ToLocalTime())} 由读者 {displayBorrower}{strBorrowRefID} 声明丢失。违约金记录id为 {strOverdueID}。最后一次借阅的情况如下: 借阅日期: {strBorrowDate}; 借阅期限: {strPeriod}。";
            }


        DOCHANGE:

            XmlElement nodeOldBorrower = null;

            // 加入到借阅历史字段中
            {
                // 看看根下面是否有borrowHistory元素
                var history_container = itemdom.DocumentElement.SelectSingleNode("borrowHistory") as XmlElement;
                if (history_container == null)
                {
                    history_container = itemdom.CreateElement("borrowHistory");
                    itemdom.DocumentElement.AppendChild(history_container);
                }
                ...
                if (this.MaxItemHistoryItems > 0)
                {
                    ...
                    // 插入到最前面
                    XmlNode temp = DomUtil.InsertFirstChild(history_container, nodeOldBorrower);
                    if (temp != null)
                    {
                        // 加入还书时间
                        DomUtil.SetAttr(temp, "returnDate", strOperTime);
                ...
                // 如果超过100个，则删除多余的
                while (history_container.ChildNodes.Count > this.MaxItemHistoryItems)
                    history_container.RemoveChild(history_container.ChildNodes[history_container.ChildNodes.Count - 1]);

                // 增量借阅量属性值
                string strBorrowCount = DomUtil.GetAttr(history_container, "count");
                if (String.IsNullOrEmpty(strBorrowCount) == true)
                    strBorrowCount = "1";
                else
                {
                ...
                DomUtil.SetAttr(history_container, "count", strBorrowCount);
            }

            if (nodeOldBorrower != null)
            {
                var borrower = DomUtil.GetElementText(itemdom.DocumentElement, "borrower");
                nodeOldBorrower.SetAttribute("barcode", borrower);
                // 2025/4/15
                // 最新版优化了一下：当读者记录只有参考 ID 没有证条码号的时候，参考 ID 放入这里的 barcode 属性(带有 "@refID:" 前缀)和 refID 属性；而如果读者记录又有参考 ID 又有证条码号，则 barcode 属性放证条码号，refID 属性放参考 ID
                if (borrower != null
                    && borrower.StartsWith("@refID:") == true)
                {
                    nodeOldBorrower.SetAttribute("refID", GetRefID(borrower));
                    // 优化 barcode 属性值
                    string readerBarcode = DomUtil.GetElementText(readerdom.DocumentElement,
                        "barcode");
                    if (string.IsNullOrEmpty(readerBarcode) == false)
                        nodeOldBorrower.SetAttribute("barcode", readerBarcode);
                }
            }
...
"overflow");
    ...
    "operator");
        ...
        strState);
            ...
            // return_info.ItemBarcode = strItemKey;
            // 2024/1/29
            ...
            out string strError)
        {
            ...
            string strItemRefID = DomUtil.GetElementText(itemdom.DocumentElement,
                "refID");
            string strItemKey = GetUnionRefID(strItemBarcode, strItemRefID);   // strItemKey;
            // 用于显示的册条码号
            string strItemDisplayKey = GetUnionBarcode(strItemBarcode, strItemRefID);
            ...
            {
                // text-level: 内部错误
                strError = "读者记录中读者证条码号和参考ID不允许同时为空";
                return -1;
            }
            ...
            if (bRenew == true)
            {
                if (nodesReservationRequest.Count > 0)
                {
                    ...
                    if (sessioninfo.UserType == "reader")
                    {
                        ...
                        // "续借操作被拒绝。因 册 " + strItemKey + " 当前已被 " + nodesReservationRequest.Count.ToString() + " 位读者预约。为方便他人，请尽早还回此书，谢谢。";
                    }
                    else
                    {
                        // text-level: 用户提示
                        strError = string.Format(this.GetString("续借操作被拒绝。因册s当前已被下列读者预约s"),  // "续借操作被拒绝。因 册 {0} 当前已被下列读者预约: {1}。为方便他人，请尽早还回此书，谢谢。"
                            strItemDisplayKey,
                            strList);
                        // "续借操作被拒绝。因 册 " + strItemKey + " 当前已被下列读者预约: " + strList + "。为方便他人，请尽早还回此书，谢谢。";
                    }
        ...
        REDO_LOAD_QUEUE_REC:
            ...
            else
            {
                    ...
                    || strNotifyReaderRefID == strReaderRefID)
                {
                    ...
                    // 在册记录中，删除相关的<reservations/request>元素
                    XmlNodeList nodes = itemdom.DocumentElement.SelectNodes($"reservations/request[@reader='@refID:{strReaderRefID}']");
                    if (nodes.Count == 0 && string.IsNullOrEmpty(strReaderBarcode) == false)
                        nodes = itemdom.DocumentElement.SelectNodes($"reservations/request[@reader='{strReaderBarcode}']");
                ...
                else // 不是预约者来借书
                {
                    ...
                    if (StringUtil.IsInList("#reservation", strLocation) == false)
                    {
                        ...
                        // "借阅操作被拒绝。因为 册 " + strItemKey + " 为读者 " + strNotifyReaderBarcode + " 所(在架)预约，已处于保留和通知取书状态。\r\n图书馆员请注意：虽然本次借阅操作被拒绝，但此册地点信息已被软件自动修改为在预约保留架(而不是在原来的普通架)，请收下此书后放入预约保留架(的特定位置，例如“曾在架”部分)。";
                        return 3;
                    }
                    ...
                    strError = string.Format(this.GetString("借阅操作被拒绝。因为册s为读者s所预约，已处于保留和通知取书状态"),
                        ...
                        strItemDisplayKey,
                        strNotifyReaderBarcode);
                    // "借阅操作被拒绝。因为 册 " + strItemKey + " 为读者 " + strNotifyReaderBarcode + " 所预约，已处于保留和通知取书状态。";
                    return 1;
                }
            }
    ...
    "refID");
            ...
            */
            ...
            {
                // text-level: 内部错误
                strError = "读者记录中读者证条码号和参考 ID 不允许同时为空";
                return -1;
            }

            {
                // 看看是否已经有先前已经借阅的册
                /*
                XmlNode nodeBorrow = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@barcode='@refID:" + strItemRefID + "']");
                if (nodeBorrow == null && string.IsNullOrEmpty(strItemBarcode) == false)
                    nodeBorrow = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@barcode='" + strItemBarcode + "']");
                */
                var nodeBorrow = LookupBorrowElement(readerdom,
                    strItemRefID,
    ...
    "refID");
            ...
            // 册条码号
            string strItemBarcode = DomUtil.GetElementText(itemdom.DocumentElement,
                "barcode"); // GetItemBarcode(itemdom, out strError);// 注: strItemKey 里面为册条码号或者册的参考 ID。注意，优先为册条码号
            //if (strItemKey == null)
            //    return -1;
            ...
            {
                // 看看是否已经有先前已经借阅的册
                nodeBorrow = LookupBorrowElement(readerdom,
                    strItemRefID,
                    strItemBarcode);
                /*
                // 2024/1/29
                ...
                if (nodeBorrow == null) // 改用册条码号。为兼容性考虑
                    nodeBorrow = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@barcode='" + strItemKey + "']") as XmlElement;
                */
                if (nodeBorrow == null)
                {
                    // text-level: 用户提示
            ...
            else // bRenew == false
            {
                ...
                nodeBorrow = (XmlElement)DomUtil.InsertFirstChild(root, nodeBorrow); // 2006/12/24 changed，2015/1/12 增加等号左边的部分 
                                                                                     // nodeBorrow = history_container.AppendChild(nodeBorrow);
            }
        ...
        SKIP_CHECK_RENEW_PERIOD:
            ...
            {
                if (bForce == false
                    && String.IsNullOrEmpty(strOldReaderKey) == false)
                {
                    // 2007/1/2
                    if (strOldReaderKey == strReaderBarcode
            ...
            var borrower_element = DomUtil.SetElementText(itemdom.DocumentElement,
                "borrower", "@refID:" + strReaderRefID /*strReaderKey*/);
            // 2025/4/15
            // borrow 元素中用 barcode 属性记载读者证条码号
            if (string.IsNullOrEmpty(strReaderBarcode) == false)
                borrower_element.SetAttribute("barcode", strReaderBarcode);
...
strBiblioRecPath);
    ...
    borrowID);
            ...
            // borrow_info.ItemBarcode = strItemKey;
            // 2024/1/29
    ...
    out DateTime lastTime) == false)
        ...
        // parameters:
        //      strItemKey  册条码号。支持 "@refID:" 前缀用法
        //      bMaskLocationReservation    不要给册记录<location>打上#reservation标记
        //      strReservationReaderKey 返回下一个预约读者的证条码号
            ...
            out string strError)
        {
            ...
            // 2024/1/30
            // 将 strItemKey 内容翻译为 @refID:xxx 形态
...
out strError);
        ...
        out strError);
                    ...
                    if (IsInBarcodeList(strItemRefIdString/*strItemKey*/,
        ...
        public LibraryServerResult DevolveReaderInfo(
            SessionInfo sessioninfo,
            string strSourceReaderKey,
            string strTargetReaderKey)
        {
            string strError = "";
            int nRet = 0;
            long lRet = 0;
            ...
            // 检查源和目标条码号不能相同
            if (strSourceReaderKey == strTargetReaderKey)
            {
                strError = "源和目标读者记录键不能相同";
                goto ERROR1;
            }

            if (String.IsNullOrEmpty(strSourceReaderKey) == true)
            {
                strError = "源读者记录键不能为空";
                goto ERROR1;
            }

            if (String.IsNullOrEmpty(strTargetReaderKey) == true)
            {
                strError = "目标读者记录键不能为空";
                goto ERROR1;
            }
            ...
            string strBarcode1 = "";
            string strBarcode2 = "";
            if (String.Compare(strSourceReaderKey, strTargetReaderKey) < 0)
            {
                strBarcode1 = strSourceReaderKey;
                strBarcode2 = strTargetReaderKey;
            }
            else
            {
                strBarcode1 = strTargetReaderKey;
                strBarcode2 = strSourceReaderKey;
            }
            ...
            try
            {
                ...
                try // 读者记录锁定1范围开始
                {
                    ...
                    try // 读者记录锁定2范围开始
                    {
                        ...
                        {
                            result.Value = -1;
                            result.ErrorInfo = "源读者证条码号 '" + strSourceReaderKey + "' 不存在";
                        ...
                        if (nRet > 1)
                        {
                            strError = "读入源读者记录时，发现读者证条码号 " + strSourceReaderKey + " 命中 " + nRet.ToString() + " 条，这是一个严重错误，请系统管理员尽快处理。";
                            goto ERROR1;
                        }
                    ...
                    out strSourceLibraryCode) == false)
                        ...
                        string source_reader_refid = DomUtil.GetElementText(source_readerdom.DocumentElement,
                            "refID");
                        string source_reader_barcode = DomUtil.GetElementText(source_readerdom.DocumentElement,
                            "barcode");
                        string source_reader_key = dp2StringUtil.BuildReaderKey(source_reader_barcode, source_reader_refid);
                        ...
                        {
                            result.Value = -1;
                            result.ErrorInfo = "目标读者证条码号 '" + strTargetReaderKey + "' 不存在";
                        ...
                        if (nRet > 1)
                        {
                            strError = "读入目标读者记录时，发现读者证条码号 " + strTargetReaderKey + " 命中 " + nRet.ToString() + " 条，这是一个严重错误，请系统管理员尽快处理。";
                            goto ERROR1;
                        }
                    ...
                    out strTargetLibraryCode) == false)
                        ...
                        string target_reader_refid = DomUtil.GetElementText(target_readerdom.DocumentElement,
    "refID");
                        string target_reader_barcode = DomUtil.GetElementText(target_readerdom.DocumentElement,
    "barcode");
                        string target_reader_key = dp2StringUtil.BuildReaderKey(target_reader_barcode, target_reader_refid);

                        if (string.IsNullOrEmpty(source_reader_refid) == false
                            && string.IsNullOrEmpty(target_reader_refid) == false
                            && source_reader_refid == target_reader_refid)
                        {
                            strError = $"源读者记录和目标读者记录的参考 ID 不允许相同";
                            goto ERROR1;
                        }
    ...
    strSourceLibraryCode + "," + strTargetLibraryCode);    // 读者所在的馆代码
                        ...
                        // 目标读者证条码号，就足以复原
                        if (string.IsNullOrEmpty(source_reader_barcode) == false)
                            DomUtil.SetElementText(domOperLog.DocumentElement,
                                "sourceReaderBarcode",
                                source_reader_barcode);
                        if (string.IsNullOrEmpty(target_reader_barcode) == false)
                            DomUtil.SetElementText(domOperLog.DocumentElement,
                            "targetReaderBarcode",
                            target_reader_barcode);
        ...
        "sourceReaderRefID",
        source_reader_refid);
                        if (string.IsNullOrEmpty(target_reader_refid) == false)
                            DomUtil.SetElementText(domOperLog.DocumentElement,
                            "targetReaderRefID",
                            target_reader_refid);
                ...
                sessioninfo.UserID);
                        ...
                        try // 在此范围内，需注意最后删除临时文件
                        {
                            ...
                            nRet = DevolveBorrowInfo(
                                channel,
                                source_reader_key,  // strSourceReaderKey,
                                target_reader_key,  // strTargetReaderKey,
                                strOperTimeString,
        ...
        // 注: 本函数根据移动的 borrow 元素多寡，决定是否使用 attachment
        // parameters:
            ...
            out string strError)
        {
            ...
            foreach (XmlElement source in nodesSourceBorrow)
            {
                ...
                // 增加一个注释元素
                DomUtil.SetAttr(target, "devolveComment", "从读者 " + strSourceReaderKey + " 转移而来，操作时间 " + strOperTimeString);

                string strEntityBarcode = DomUtil.GetAttr(source, "barcode");
                string strEntityRefID = source.GetAttribute("refID");
                if (String.IsNullOrEmpty(strEntityBarcode) == true
                    && string.IsNullOrEmpty(strEntityRefID) == true)
                    continue;

                string strEntityKey = dp2StringUtil.BuildReaderKey(strEntityBarcode, strEntityRefID);
                ...
                nRet = ChangeEntityBorrower(
                    ...
                    strSourceReaderKey, // strSourceReaderKey,
                    strTargetReaderKey, // strTargetReaderKey,
                    strOperTimeString,
                    ref domOperLog,
                    attachment,
            ...
            var nodeComment = source_dom.CreateElement("devolvedBorrows");
            source_dom.DocumentElement.AppendChild(nodeComment);
            nodeComment.InnerXml = nodeSourceBorrows.InnerXml;

            DomUtil.SetAttr(nodeComment, "comment", "已于 " + strOperTimeString + " 将下列借阅信息转移到读者 " + strTargetReaderKey + " 名下");
            ...
            out string strError)
        {
            ...
            DomUtil.SetAttr(nodeComment, "comment", "已于 "
                + strOperTimeString
                + " 将下列超期信息转移到读者 " + strTargetReaderKey + " 名下");
        ...
        // 修改册记录中的借者证条码号
        // parameters:
        //      strEntityKey    册记录检索键。内容为册条码号，或者 "@refID:xxx" 形态
        //      strOldReaderKey 旧的读者键。内容为证条码号，或者 "@refID:xxx" 形态
        //      strNewReaderKey 新的读者键。注意，(虽然也可以使用两种形态，但)倾向于尽量使用 "@refID:xxx" 形态
        ...
        int ChangeEntityBorrower(
            RmsChannel channel,
            string strEntityKey,
            string strOldReaderKey,
            string strNewReaderKey,
            string strOperTimeString,
            ref XmlDocument domOperLog,
            Stream attachment,
            ...
            out string strError)
        {
            ...
            if (String.IsNullOrEmpty(strOldReaderKey) == true)
            {
                strError = "strOldReaderKey 参数值不能为空";
                goto ERROR1;
            }
...
out strError);
            if (nRet == -1)
                ...
                strOldReaderRefID = GetRefID(strOldReaderRefID);

            nRet = this.ConvertRefIdListToReaderBarcodeList(channel,
strOldReaderKey,
out string strOldReaderBarcode,
out strError);
            if (nRet == -1)
                strOldReaderBarcode = "";

            // 加册记录锁
            this.EntityLocks.LockForWrite(strEntityKey);
            ...
            REDO:
                ...
                if (nRet == 0)
                {
                    strError = "册条码号 '" + strEntityKey + "' 不存在";
                    return 0;
                }
                if (nRet == -1)
                {
                    strError = "读入册记录 '" + strEntityKey + "' 时发生错误: " + strError;
                    goto ERROR1;
                }
                ...
                {
                    /*
                    strError = "册条码号为 '" + strEntityKey + "' 的册记录有 " + aPath.Count.ToString() + " 条，无法进行修改";
                    return -1;
                     * */
                    ...
                    nRet = FindItem(
                        channel,
                        strOldReaderBarcode,
                        strOldReaderRefID, // TODO: 将来考虑增加参考 ID 参数
                        aPath,
                        true,   // 优化
                        out aFoundPath,
                        out aItemXml,
                    ...
                    if (nRet == 0)
                    {
                        strError = "册条码号 '" + strEntityKey + "' 检索出的 " + aPath.Count + " 条记录中，没有任何一条其<borrower>元素表明了被读者 '" + strOldReaderKey + "' 借阅。";
                        goto ERROR1;
                    }

                    if (nRet > 1)
                    {
                        strError = "册条码号为 '" + strEntityKey + "' 并且<borrower>元素表明为读者 '" + strOldReaderKey + "' 借阅的册记录有 " + aFoundPath.Count.ToString() + " 条，无法进行移动操作。";
                        goto ERROR1;
                    }
                ...
                if (strBorrower != strOldReaderBarcode
                    && strBorrower != $"@refID:{strOldReaderRefID}")
                {
                ...
                DomUtil.SetElementText(itemdom.DocumentElement,
                    ...
                    + strOperTimeString + " 被转移到读者 " + strNewReaderKey + " 名下");
            ...
            {
                this.EntityLocks.UnlockForWrite(strEntityKey);
            }
            return 1;
        ERROR1:
            return -1;
            ...
            out int nTotalBorrowItems)
        {
            ...
            try // 读者记录锁定范围开始
            {
                ...
                {
                    result.Value = -1;
                    result.ErrorInfo = "读者证条码号 '" + PatronBarcodeLink(strReaderKey) + "' 不存在";
                    result.ErrorCode = ErrorCode.ReaderBarcodeNotFound;
                    return result;
                }
                if (nRet == -1)
                ...
                if (nRet > 1)
                {
                    strError = $"读入读者记录时发生错误: 读者键 '{strReaderKey}' 检索命中多条记录({nRet})";
                    goto ERROR1;
                }
                ...
                var readerBarcode = DomUtil.GetElementText(readerdom.DocumentElement,
                    "barcode");
                var readerRefID = DomUtil.GetElementText(readerdom.DocumentElement,
                    "refID");

                List<XmlElement> nodesBorrow = null;
                if (string.IsNullOrEmpty(strFilterItemKey))
                    nodesBorrow = new List<XmlElement>(
                        readerdom.DocumentElement.SelectNodes("borrows/borrow").Cast<XmlElement>()
                        );
                else
                {
                    // 读入册记录
    ...
    out strError);
                ...
                // TODO: 检查 borrow 元素的 refID 属性值的重复情况
                ...
                for (int i = nStart; i < nTotalBorrowItems; i++)
                {
                    ...
                    var node = nodesBorrow[i] as XmlElement;

                    string strItemBarcode = DomUtil.GetAttr(node, "barcode");
                    // 2024/2/20
                    string strItemRefID = node.GetAttribute("refID");
                    string strItemKey = dp2StringUtil.BuildReaderKey(strItemBarcode, strItemRefID);
                    ...
                    LibraryServerResult result_1 = CheckItemBorrowInfo(
                        ...
                        strItemKey, // strItemKey,
                        null,
                        out string strOutputReaderBarcode_0,
                        out string[] aDupPath);
                    if (result_1.Value == -1 || result_1.Value == 1)
                    {
                        if (result_1.ErrorCode == ErrorCode.ItemBarcodeDup)
                        {
                            ...
                            for (int j = 0; j < aDupPath.Length; j++)
                            {
                                ...
                                // TODO: 两个 key 都不是确定的形态
                                // if (strOutputReaderBarcode == strReaderKey)
                                if (IsReaderKeySame(channel,
                                    strOutputReaderBarcode,
                                    strReaderKey))
                                {
                        ...
                        {
                            strCheckError += "读者记录中借阅册条码号 " + ItemBarcodeLink(strItemBarcode) + " 关联的册记录中，其 borrower 元素关联回的读者证条码号是 '" + PatronBarcodeLink(strOutputReaderBarcode_0) + "'，而不是出发的读者证条码号 '" + PatronBarcodeLink(strReaderKey) + "'。并且证条码号为 '" + PatronBarcodeLink(strOutputReaderBarcode_0) + "' 的读者记录不存在。";
                            nErrorCount++;
                            continue;
                        }
                    ...
                    // if (strOutputReaderBarcode_0 != strReaderKey)
                    if (IsReaderKeySame(channel,
    ...
    out error);
        ...
        out strError);
            ...
            {
                result.Value = -1;
                result.ErrorInfo = "册条码号 '" + ItemBarcodeLink(strItemKey) + "' 不存在";
                result.ErrorCode = ErrorCode.ItemBarcodeNotFound;
                return result;
            }
            if (nRet > 1)
            {
                result.Value = -1;
                result.ErrorInfo = "册条码号为 '" + ItemBarcodeLink(strItemKey) + "' 的册记录有 " + nRet + " 条，无法进行修复操作。请在附加册记录路径后重新提交修复操作。";
                result.ErrorCode = ErrorCode.ItemBarcodeDup;
    ...
    "borrower");
            ...
            try // 读者记录锁定范围开始
            {
                ...
                if (exist_readerdom == null)
                {
                    string key = string.IsNullOrEmpty(strLockedReaderBarcode) == false ? strLockedReaderBarcode : strOutputReaderBarcode;
                    nRet = this.GetReaderRecXml(
                        channel,
                        key,
                        out strReaderXml,
                    ...
                    {
                        result.Value = 1;
                        result.ErrorInfo = "读者证条码号 '" + PatronBarcodeLink(key) + "' 不存在";
                        result.ErrorCode = ErrorCode.ReaderBarcodeNotFound;
                        return result;
                    }
                    if (nRet == -1)
                    ...
                    {
                        result.Value = 1;
                        result.ErrorInfo = $"读者证条码号 '{PatronBarcodeLink(key)} ' 检索命中多条记录 ({nRet})，无法进行修复操作。";
                        if (GetKeyType(key) == "barcode")
                            result.ErrorInfo += "可改用读者参考 ID 重新请求";
                ...
                var nodesBorrow = LookupBorrowElements(readerdom,
                    strItemRefID,
                    strItemBarcode);
                if (nodesBorrow.Count == 0)
                {
                    strCheckError += "虽然册记录 " + ItemRecPathLink(strOutputItemRecPath) + " 中表明了被读者 '" + PatronBarcodeLink(strOutputReaderBarcode) + "' 借阅，但是读者记录 " + PatronRecPathLink(strOutputReaderRecPath) + " 中并没有关于册条码号 '" + ItemBarcodeLink(strItemKey) + "' 的借阅记录。";
                    goto END1;
                }
                if (nodesBorrow.Count > 1)
                {
                    strCheckError = "读者记录 " + PatronRecPathLink(strOutputReaderRecPath) + " 中有关于册条码号 '" + ItemBarcodeLink(strItemKey) + "' 的 " + nodesBorrow.Count.ToString() + " 条含糊借阅记录(建议从读者一侧加以修复)。";
                    goto END1;
                }
#if REMOVED
                XmlNodeList nodesBorrow = readerdom.DocumentElement.SelectNodes("borrows/borrow[@refID='" + strItemRefID + "']");
                if (nodesBorrow.Count == 0)
                    nodesBorrow = readerdom.DocumentElement.SelectNodes("borrows/borrow[@barcode='" + strItemBarcode + "']");
                if (nodesBorrow.Count == 0)
                    nodesBorrow = readerdom.DocumentElement.SelectNodes("borrows/borrow[@barcode='@refID:" + strItemRefID + "']");
                if (nodesBorrow.Count == 0)
                {
                    strCheckError += "虽然册记录 " + ItemRecPathLink(strOutputItemRecPath) + " 中表明了被读者 '" + PatronBarcodeLink(strOutputReaderBarcode) + "' 借阅，但是读者记录 " + PatronRecPathLink(strOutputReaderRecPath) + " 中并没有关于册条码号 '" + ItemBarcodeLink(strItemKey) + "' 的借阅记录。";
                    goto END1;
                ...
                if (nodesBorrow.Count > 1)
                {
                    strCheckError = "读者记录 " + PatronRecPathLink(strOutputReaderRecPath) + " 中有关于册条码号 '" + ItemBarcodeLink(strItemKey) + "' 的 " + nodesBorrow.Count.ToString() + " 条含糊借阅记录。";
                    goto END1;
                }
#endif
        ...
        //      strItemKey      册键。不允许为空
        //      style       skipOperLog 跳过写入操作日志
        //                  version:xxx 日志记录的版本号。本函数用于日志恢复时，使用此信息
        // result.Value
        //      -1  出错
        //      0   没有必要修复
        //      1   成功修复
            ...
            out string[] aDupPath)
        {
            ...
            if (String.IsNullOrEmpty(strReaderKey) == true)
            {
                strError = "strReaderKey 参数不能为空";
                goto ERROR1;
            }
            if (String.IsNullOrEmpty(strItemKey) == true)
            {
                strError = "strItemKey 参数不能为空";
                goto ERROR1;
            }
        ...
        REDO_REPAIR:
            ...
            try // 读者记录锁定范围开始
            {
                ...
                // 注意：strReaderKey 内容可以使用 @refID: 前缀
                nRet = this.GetReaderRecXml(
                    // sessioninfo.Channels,
                    channel,
                    strReaderKey,
            ...
            out strError);
                ...
                // 2024/4/23
                // 先尝试删除因参考 ID 重复而多余的 borrow 元素
                int remove_borrow_count = RemoveDupBorrow(readerdom);

                // 预先用 strItemKey 权且搜索一下 borrow 元素
                /*
                XmlNode nodeBorrow = readerdom.DocumentElement.SelectSingleNode($"borrows/borrow[@barcode='{strItemKey}']");
                if (nodeBorrow == null)
                {
                    if (strItemKey.StartsWith("@refID:"))
                        nodeBorrow = readerdom.DocumentElement.SelectSingleNode($"borrows/borrow[@refID='{dp2StringUtil.GetRefIdValue(strItemKey)}']");
                    else
                        nodeBorrow = readerdom.DocumentElement.SelectSingleNode($"borrows/borrow[@barcode='{strItemKey}']");
                }
                */
    ...
    out bool exactly);
                    ...
                    else
                    {
                        ...
                        {
                            result.Value = -1;
                            result.ErrorInfo = "册条码号为 '" + ItemBarcodeLink(strItemKey) + "' 的册记录有 " + aPath.Count.ToString() + " 条，无法进行修复操作。请在附加册记录路径后重新提交修复操作。";
                            result.ErrorCode = ErrorCode.ItemBarcodeDup;
                    ...
                    strItemBarcode = DomUtil.GetElementText(itemdom.DocumentElement,
                        "barcode");
                    strItemRefID = DomUtil.GetElementText(itemdom.DocumentElement,
                        "refID");

                    // 在得到册记录参考 ID 后，再次定位 borrow 元素
                    if (nodeBorrow == null)
                    {
                        nodeBorrow = LookupBorrowElement(readerdom,
                            strItemRefID,
                            strItemBarcode);
                        /*
                        nodeBorrow = readerdom.DocumentElement.SelectSingleNode($"borrows/borrow[@barcode='@refID:{strItemRefID}']");
                        if (nodeBorrow == null && string.IsNullOrEmpty(strItemKey) == false)
                            nodeBorrow = readerdom.DocumentElement.SelectSingleNode($"borrows/borrow[@barcode='{strItemKey}']");
                        if (nodeBorrow == null && string.IsNullOrEmpty(strItemRefID) == false)
                ...
                DELETE_CHAIN:
                    ...
                    // var borrowRefID = nodeBorrow.GetAttribute("refID");
                ...
                /*
                // 2024/4/23
                // 根据参考 ID 进行搜索，删除所有匹配这个参考 ID 的 borrow 元素
                if (string.IsNullOrEmpty(borrowRefID) == false)
                {
                    var nodes = readerdom.DocumentElement.SelectNodes($"borrows/borrow[@refID='{borrowRefID}']");
                    foreach(XmlElement borrow in nodes)
                    {
                ...
                */
                ...
                WRITE_READER_RECORD:
                    ...
                    string changed_reader_xml = readerdom.DocumentElement?.OuterXml;
                    ...
                    string changed_item_xml = "";
                    bool item_xml_changed = false;

                    // 注意 itemdom 可能为 null
                    string old_item_xml = itemdom?.DocumentElement?.OuterXml;

                    string version = StringUtil.GetParameterByPrefix(style, "version");
                    // 注: version 1.10 和以上版本的日志动作执行时，暂时去掉这部分功能。不改变册记录。如果 style 中不包含 version:xxx 子参数，理解为最新版，也就是说 version 在 1.10 以上
                    // TODO: 建议把册记录中下列残余元素给清除:
                    /*
    <borrowerReaderType>本科生</borrowerReaderType>
    <borrowerRecPath>读者/1</borrowerRecPath>
    <borrowDate>Tue, 20 Feb 2024 22:12:15 +0800</borrowDate>
    <borrowID>98aa0de4-7530-4873-82ea-95ff68e707dc</borrowID>
    <borrowPeriod>31day</borrowPeriod>
    <returningDate>Fri, 22 Mar 2024 12:00:00 +0800</returningDate>
    <operator>supervisor</operator>
                    * */
                    // 2024/2/22
                    if (itemdom != null
                        && (version != null && StringUtil.CompareVersion(version, "1.10") < 0))
                    {
                        // 移除册记录侧的链
                        ClearBorrower(itemdom);

                        DomUtil.RemoveEmptyElements(itemdom.DocumentElement);

                        changed_item_xml = itemdom.DocumentElement?.OuterXml;

                        if (old_item_xml != changed_item_xml)
                        {
                            item_xml_changed = true;
                            // 写回册记录
                            lRet = channel.DoSaveTextRes(strOutputItemRecPath,
                                itemdom.OuterXml,
                                false,
                    ...
                    // 如果册记录中 borrower 元素有值，则需要在返回时警告“册一侧还需要修复”
        ...
        <!-- 2024/2/22 增加 -->
        <readerRecord recPath='...'>...</readerRecord> 如果读者记录发生了改变，则这里记载改变后的册记录内容
        <itemRecord recPath='...'>...</itemRecord> 如果册记录发生了改变，则这里记载改变后的册记录内容
        </history_container>
                         * * */
        ...
        strLibraryCode);    // 读者所在的馆代码
                        DomUtil.SetElementText(domOperLog.DocumentElement,
                            "operation", "repairBorrowInfo");
                        DomUtil.SetElementText(domOperLog.DocumentElement,
                            "action", "repairreaderside");
                        DomUtil.SetElementText(domOperLog.DocumentElement,
                        ...
                        // if (string.IsNullOrEmpty(changed_reader_xml) == false)
                        {
                            var element = DomUtil.SetElementText(domOperLog.DocumentElement,
                                "readerRecord",
                                changed_reader_xml);
...

                        {
                            ...
                            var element = DomUtil.SetElementText(domOperLog.DocumentElement,
                                ...
                                xml);
                            if (string.IsNullOrEmpty(strOutputItemRecPath) == false)
                                element.SetAttribute("recPath", strOutputItemRecPath);
                            if (item_xml_changed == false)
                                element.SetAttribute("changed", "false");
                            if (string.IsNullOrEmpty(xml)
                                || string.IsNullOrEmpty(strOutputItemRecPath))
                            {
                                element.SetAttribute("existing", "false");
                                if (string.IsNullOrEmpty(strItemKey) == false)
                                    element.SetAttribute("itemKey", strItemKey);
                            }
                        }
        ...
        sessioninfo.UserID);
                ...
                {
                    // 解册记录锁
                    this.EntityLocks.UnlockForWrite(strItemKey);
                }
            }
            finally
            {
        ...
        // 删除 borrow 元素之间重复参考 ID 的那部分元素
        int RemoveDupBorrow(XmlDocument readerdom)
        {
            int count = 0;
            // 获得所有参考 ID
            List<string> refids = new List<string>();
            {
                ...
                foreach (XmlElement borrow in nodes)
                {
                    var refid = borrow.GetAttribute("refID");
            ...
            {
                var nodes = readerdom.DocumentElement.SelectNodes($"borrows/borrow[@refID='{refid}']");
                for (int i = 1; i < nodes.Count; i++)
                {
                    var borrow = nodes[i] as XmlElement;
                    borrow.ParentNode.RemoveChild(borrow);
                    count++;
                }
            }
        ...
        static bool IsEqual(string xml1, string xml2)
        {
        ...
        // 用 册 Key 匹配读者记录的 borrow 元素
        // 算法是根据 册 Key 的类型，和 borrow/@barcode 或者 borrow/@refID 匹配
        // 注意，如果 册 Key 是册条码号，可能会匹配到较为陈旧的 borrow/@barcode 内容
        // parameters:
        //      strItemKey  可能是册条码号，也可能是 "@refID:xxx" 形态
        //      exactly     [out] 是否精确匹配？如果是一个 borrow 元素同时具有 refID 和 barcode 属性，匹配上了 barcode 属性值，返回 exactly = false。其余情况返回 exactly = true
        static XmlElement LookupBorrowElement(XmlDocument readerdom,
            string strItemKey,
            out bool exactly)
        {
            exactly = true;
            ...
            foreach (XmlElement borrow in nodes)
            {
                var barcode = borrow.GetAttribute("barcode");
                var refID = borrow.GetAttribute("refID");
                if (string.IsNullOrEmpty(refID) == false)
                {
                    if (strItemKey.StartsWith("@refID:")
                        && dp2StringUtil.GetRefIdValue(strItemKey) == refID)
                    {
                        exactly = true;
                        return borrow;
                    }
                ...
                {
                    // 注意早期 barcode 属性值可能是 "@refID:xxx" 形态
                    if (barcode == strItemKey)
                    {
                        if (string.IsNullOrEmpty(refID) == false
                            && strItemKey.StartsWith("@refID:") == false)
                            exactly = false;
                        else
                            exactly = true;
                        return borrow;
    ...
    string itemBarcode)
        {
            ...
            foreach (XmlElement borrow in nodes)
            {
                ...
                {
                    // 注意早期 barcode 属性值可能是 "@refID:xxx" 形态
                    if (barcode.StartsWith("@refID:")
                        && barcode == $"@refID:{itemRefID}")
                        return borrow;
                    if (barcode.StartsWith("@refID:") == false
                        && barcode == itemBarcode)
                        return borrow;
                }
            }
        ...
        static List<XmlElement> LookupBorrowElements(XmlDocument readerdom,
string itemRefID,
string itemBarcode)
        {
        ...
        /*
        // 检查 borrow 元素中 refID 和 barcode 属性值的一致性
        // return:
        //      -1  检查过程发生错误
        //      0   没有不一致
        //      1   发现了不一致
        //      2   发现了不一致，并且已经修复
        int CheckBarcodeRefIDAttribute(
            ...
            out string strError)
        {
            ...
            var refID = nodeBorrow.GetAttribute("refID");
            var barcode = nodeBorrow.GetAttribute("barcode");
        ...
        */
        ...
        // 根据册键和确认路径装载一条册记录
        // parameters:
        //      func_selectItems    命中多条时用于筛选的回调函数。
        //                          如果为 null，表示用默认的方式以 strConfirmItemRecPath 对路径进行进一步筛选
        // return:
        //      -1  出错
        //      0   没有找到
        //      其它  命中的记录条数
    ...
    out string strError)
        {
            ...
            if (aPath.Count > 1)
            {
                if (func_selectItems != null)
                {
                    int ret = func_selectItems(aPath,
                        ref strItemXml,
                        ref item_timestamp,
                    ...
                    if (ret == -1)
                    {
                        strError = $"命中记录多条({nRet})，在进一步选择的过程中出错: {error}";
                        return -1;
                    }
                    return 1;
                }
                else
                {
                    ...
                    if (string.IsNullOrEmpty(strConfirmItemRecPath) == false)
                    {
                        ...
                        {
                            if (path == strConfirmItemRecPath)
                            {
                                strOutputItemRecPath = path;
                                long lRet = channel.GetRes(path,
                                    out strItemXml,
                                    out _,
                                    out item_timestamp,
                                    out _,
        ...
        REDO_REPAIR:
            ...
            try // 读者记录锁定范围开始
            {
                ...
                {
                    strError = "证条码号为 '" + PatronBarcodeLink(strReaderKey) + "' 的读者记录存在 " + nRet + " 条。请先修复此问题，再重试修复册记录的链问题";
                    goto ERROR1;
                }
                else
                {
            ...
            out strError);
                ...
                string strReaderBarcode = readerdom == null ?
                    null : DomUtil.GetElementText(readerdom.DocumentElement,
    "barcode");
                string strReaderRefID = readerdom == null ?
                    null : DomUtil.GetElementText(readerdom.DocumentElement,
                    "refID");
            ...
            out strError);
                    ...
                    {
                        if (strBorrower != strReaderKey)
                        {
                            strError = $"修复操作被拒绝。您所请求要修复的册记录中，并没有指明借阅者是读者 '{PatronBarcodeLink(strReaderKey)}'。(实际上借阅者(borrower元素值)是 '{PatronBarcodeLink(strBorrower)}')";
                            strError += $"。注: 读者记录 '{strReaderKey}' 不存在";
                            //if (GetKeyType(strReaderKey) != GetKeyType(strBorrower))
                            //{
                            //
                            //}
                            if (GetKeyType(strReaderKey) == "barcode" && GetKeyType(strBorrower) == "refID")
                                strError += "，无法确认其参考 ID。如果可能请改用参考 ID 形态的“读者 Key”参数重新请求本功能";
                            else if (GetKeyType(strReaderKey) == "refID" && GetKeyType(strBorrower) == "barcode")
                                strError += "，无法确认其证条码号。如果可能请改用证条码号形态的“读者 Key”参数重新请求本功能";
                            else
                                strError += "(不过这一点并不重要，和解决问题无关。API 执行首先要求给出的 strReaderKey 参数值能和册记录中的 borrower 元素内容匹配，这一点很重要)";
                            goto ERROR1;
                        }
                    }
                    ...
                    if (readerdom != null)
                    {
                        var nodeBorrow = LookupBorrowElement(readerdom,
                            ...
                            strItemBarcode);
                        /*
                        XmlNode nodeBorrow = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@refID='" + strItemRefID + "']");
                        if (nodeBorrow == null)
                            nodeBorrow = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@barcode='" + strItemKey + "']");
                        if (nodeBorrow == null)
                            nodeBorrow = readerdom.DocumentElement.SelectSingleNode("borrows/borrow[@barcode='@refID:" + strItemRefID + "']");
                    ...
                    string changed_item_xml = itemdom.DocumentElement?.OuterXml;
                    // 写回册记录
                    lRet = channel.DoSaveTextRes(strOutputItemRecPath,
                        itemdom.OuterXml,
                        false,
        ...
        strLibraryCode);    // 册所在的馆代码
                    ...
                    DomUtil.SetElementText(domOperLog.DocumentElement,
                        "readerBarcode",
                        string.IsNullOrEmpty(strReaderBarcode) ? strReaderKey : strReaderBarcode);
                        */
                        ...
                        // 2024/4/11
                        // 虽然本函数不会修改读者记录，但也值得记载一下侦测到的读者记录是否存在等情况。有利于后期以快照方式恢复相关记录
                        {
                            var element = DomUtil.SetElementText(domOperLog.DocumentElement,
                                "readerRecord",
                                strReaderXml);
                            ...
                            element.SetAttribute("changed", "false");
                            if (string.IsNullOrEmpty(strReaderXml)
                                || string.IsNullOrEmpty(strOutputReaderRecPath))
                            {
                                element.SetAttribute("existing", "false");  // 表示和册记录关联的读者记录不存在
                                // 注: 如果读者记录不存在，则 readerBarcode 里面写入读者键。
                                // 读者键是前端请求本功能前，设法从册记录的 borrower 元素中得到的
                                if (string.IsNullOrEmpty(strReaderKey) == false)
                                    element.SetAttribute("readerKey", strReaderKey);
                            }
                        }

                        // if (string.IsNullOrEmpty(changed_item_xml) == false)
                        {
                            var element = DomUtil.SetElementText(domOperLog.DocumentElement,
                                "itemRecord",
                                changed_item_xml);
        ...
        // 获得一个 key 字符串的类型。注意空字符串被当作 "barcode" 类型
        ...
        public static void ClearBorrower(XmlDocument itemdom)
        {
            if (itemdom == null || itemdom.DocumentElement == null)
                return;
    ...
    strLibraryCode);
        ...
        // 创建押金交费请求
        // parameters:
        //      strReaderKey        读者证条码号，或者 "@refID:xxx" 形态
        ...
        REDO_FOREGIFT:
...
#endif
            this.ReaderLocks.LockForRead(strReaderKey);
            ...
            try // 读者记录锁定范围开始
            {
                ...
                {
                    result.Value = -1;
                    result.ErrorInfo = "读者证条码号 '" + strReaderKey + "' 不存在";
                    result.ErrorCode = ErrorCode.ReaderBarcodeNotFound;
                    return result;
                }
                if (nRet == -1)
            ...
            out strLibraryCode) == false)
                ...
                for (int i = 0; i < nodeOverdues.Count; i++)
                {
                    ...
                    if (strPart == strWord)
                    {
                        strError = "读者 '" + strReaderKey + "' 已经存在押金交费请求。需要先将此押金请求交费完成后，才能创建新的押金交费请求。";
                        goto ERROR1;
                    }
                }
    ...
    strLibraryCode);    // 读者所在的馆代码
        ...
        strReaderRefID);
            ...
            {
                this.ReaderLocks.UnlockForRead(strReaderKey);
#if DEBUG_LOCK_READER
        ...
        // 根据Foregift() API要求，修改readerdom
        // 本函数只会修改 readerdom 内容
        // parameters:
        //      strAction   为 foregift 和 return 之一
            ...
            out string strError)
        {
            ...
            // 获得相关参数
            var nodeForegift = readerdom.DocumentElement.SelectSingleNode("foregift") as XmlElement;
            /*
            ...
            */
            // 2024/4/11 注: 这里不急于添加一个空内容的 foregift 元素。等后面读者交费了以后，交费功能自然是会自动添加 foregift 元素
        ...
        REDO_HIRE:
            ...
            out strLibraryCode) == false)
                ...
                for (int i = 0; i < nodeOverdues.Count; i++)
                {
                    ...
                    if (strPart == strWord)
                    {
                        strError = "读者 '" + strReaderKey + "' 已经存在租金交费请求。需要先将此租金请求交费完成后，才能创建新的租金交费请求。";
                        goto ERROR1;
                    }
                }
    ...
    strReaderRefID);
            ...
            out string strError)
        {
            ...
            {
                // hire 动作的算法:
                // 如果当前存在上次租金失效期(hire/@expireDate)，则使用它；
                // 否则取办证日期和当前日期的靠后者
            ...
            else if (strAction == "hirelate")   // hire 和 hirelate 有什么区别? 区别是 hire 多参考了一个办证日期
            {
                // hirelate 动作的算法:
                // 取当前日期和上次租金失效期之中靠后的一个。
