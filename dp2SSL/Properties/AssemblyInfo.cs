...
[assembly: AssemblyVersion("1.9.14")]    // 1.5.*
[assembly: AssemblyFileVersion("1.9.14.0")]  // 1.5.0.0
...
//              (2024/1/4) 借书界面，刷新图书显示和读者信息显示可以并行了
//              (2024/1/5) 人脸识别命中多个的选择对话框，如果数字键盘 NUMLOCK 为 OFF 时输入，会出现红色提示
//      1.9.9   (2024/1/11) 自助借书界面，和书柜界面，可以用键盘回车启动人脸识别
//              (2024/1/12) 解决读者信息同步 setreaderinfo new 动作时没有利用 libraryCode 元素 从而产生 .xxx 形态的 UII 的一个 bug
//                          界面风格做了部分优化。左侧按钮向右挪动了一点，为了避免小尺寸书柜屏幕触摸困难。人脸识别按钮增加了中文。主菜单采用 Thin 字体 Weight
//              (2024/1/16) 解决图书封面图片残缺的文件造成崩溃的 bug。被锁定的图像临时文件会被记入 memory_deleting.txt 文件，等下次 dp2ssl 启动时会根据里面的记载删除临时图像文件(最后删除这个记忆文件)
//              (2024/1/17) 管理界面增加了立即触发盘点的命令，和一个迫使重新启动以后进行盘点的命令。都是针对书柜的功能
//              (2024/1/18) 掌纹识别功能解决了 "capture:xxx" 被当作读者证条码号装载读者的 bug
//      1.9.10  (2024/1/18) 书柜初始化阶段，增加了对每个门中的图书，和全部图书进行 UII 查重、报错的步骤
//                          书柜全部下载读者记录到本地缓存的中途如果遇到中断，settings.xml 文件中 patronReplication/startDate 参数会被清空，这样，后面重新启动 dp2ssl 的时候或者其它适当时机，会自动重新开始下载全部读者记录到本地缓存，直到全部成功为止
//                          自助借还原先的配置参数“身份读卡器竖放”被删除，代替它的是一个新的参数“竖放的身份读写器”，内容为身份读写器的名字。自助借还界面会根据这个读写器名字，判断 ISO14443A 或 ISO15693 或者 ISO18000P6C 的读者卡是否需要按照竖放进行特定处理和提醒。原来参数的缺点是，无法弄清楚两个或以上的读写器都可以读 ISO15693 读者卡、到底哪个是竖放的
//      1.9.11  (2024/1/22) 自助借还界面和书柜界面增加掌纹视频显示功能。并对掌纹识别重复输入同一个读者的证条码号情形做了控制处理
//              (2024/1/23) 自助借还界面增加检查图书标签 UII 重复功能
//                          当书柜状态，探测到 library.xml 中 rfid 元素发生变化后，早先版本会触发重新下载读者和册记录。新版本，对于册记录部分，改进为，要观察 App.ReplicateEntities 参数，如果此参数为 true 才会触发重新下载册记录，否则只会清除本地缓存的册记录(而不会引起重新下载)
//              (2024/1/24) PageSetting 中修改 App.ReplicateEntities 参数，会自动触发重新全量下载册记录到本地缓存或者清除本地缓存册记录动作
//                          下架图书的时候，会出现黄色警告对话框，警告调出的图书 EAS 状态为 Off，不能直接上架提供开架外借，而必须经过 EAS 修正手续
//              (2024/1/25) 改进书柜模式下，全量读者记录和全量册记录下载到本地的循环过程，确保两种全量下载完成之后才会进行零星同步
//                          biblio.db 中的 entities 表结构，增加了一个字段 LastWriteTime，内容为记录最后修改的时间
//      1.9.12  (2024/1/30) 书柜状态，断网状态，人脸识别命中多个，显示读者信息的列表会长期转圈动画等待。已经改为直接利用本地缓存的读者信息显示。
//                          书柜状态，刷一个没有绑定的 ISO14443A 读者卡，读者信息画面(报错状态)没有自动延时清除。这个 bug 已经修正。
//                          书柜状态，断网状态，用手机我爱图书馆微信公众号的读者二维码扫码，虽然会报错说找不到读者记录，但报错信息增加了更友好的提示内容“断网状态下 ...”
//              (2024/1/31) biblio.db 中的 entities 表结构，增加了一个字段 ImageUrl，内容为封面图片的对象路径，为绝对路径形态，或 http https 形态
//                          书柜状态，点书柜界面上的门数字，出现的图书列表对话框中增加了图书封面显示。在联网和断网状态下都可以显示出封面。断网状态下只能显示原先在联网状态下在书柜中出现过的图书的封面
//                          书柜状态下，断网状态，注册人脸 和 绑定读者卡，这两个功能都无法使用，最新版增加了警告对话框
//                          书柜状态下，注册人脸 和 绑定读者卡页面右侧的读者信息区，刷卡的时候会把读写器当作竖放进行延时自动清除处理。先前版本这里会受到“竖放的读写器名字”系统参数影响，最新版已经和这个系统参数无关，只要是书柜状态就默认读者卡读写器是竖放
//              (2024/2/2)  书柜状态下，借书还书和上架下架的提示对话框里面为每册图书增加了封面显示
//              (2024/2/4)  书柜状态下，借书还书和上架下架的提示对话框修正几个 bug。
//                          SubmitDocument 中 Image 控件会在 Build() 和 Refresh() 之间重复使用，这样可以减少内存占用
...
//      1.9.14  (2024/4/26) 判断望湖洞庭格式，增加了一种 AFI == 4 的情形
//                          设置参数增加了“只读 EPC”加速的这一种参数