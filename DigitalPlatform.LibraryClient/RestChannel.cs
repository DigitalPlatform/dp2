    ...
    out MessageData[] output_messages)
        {
            ...
            byte[] result = this.UploadData(GetRestfulApiUrl(this.ServerUrl, "SetMessage"),
                "POST",
                baData);
            ...
            dynamic response = JsonConvert.DeserializeObject(strResult);
#else
            var response = JsonConvert.DeserializeObject<SetMessageResponse>(strResult);
#endif
            output_messages = response.output_messages;
            return response.SetMessageResult;
        }
