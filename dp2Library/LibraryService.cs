...
namespace dp2Library
{
    ...
    strVia);
        ...
        //      Value == 0  正常
        //      Value == -1 不正常
        LibraryServerResult PrepareEnvironment(bool bPrepareSessionInfo,
        ...
        //              如果是public登录，type应该是worker，不能是reader。因为reader会被程序把strUserName当成条码号来登录。public登录成功后，身份还是reader
        //      bReader 身份是否为读者
        // return: result.Value:
        //      -1  error
            ...
            out string strLibraryCode)
        {
            ...
            try
            {
                ...
                if (bReader == false)
                {
                    ...
                    {
                        strError = "strParameters中type子参数值为reader以外类型时，不允许进行simulate方式的登录";
                        goto ERROR1;
                    }
                    // 工作人员登录
                ...
                else
                {
                    if (bSimulateLogin == true)
                    {
                        ...
                        {
                            strError = "simulate状态下登录时，strPassword参数不能为空";
                            goto ERROR1;
                        }
                        ...
                        try
                        {
                            string strTemp = "";
                            // 工作人员登录
                            nRet = sessioninfo.Login(strManagerUserName,
                                 strManagerPassword,
                                 "#simulate",
                                 false,
                                 out strRights,
                            ...
                            if (nRet != 1)
                            {
                                strError = "simulate登录时第一阶段利用管理员帐户验证登录失败: " + strError;
                                goto ERROR1;
                            }
                        }
                        finally
        ...
        {
            LibraryServerResult result = this.PrepareEnvironment(true, true, true);
            if (result.Value == -1)
                return result;
    ...
    email* 
        ...
        // 列出 书目库/读者库/订购库/期库/评注库/发票库/违约金库 检索途径信息
        // parameters:
        //      strLang 语言代码。一般为"zh"
        //      infos   返回检索途径信息数组
        // rights:
            ...
            GET_OTHERINFO:
                ...
                if (sessioninfo.GlobalUser == false) // 分馆用户必须要过滤，因为要修改<borrower>
                {
...

                    {
                        ...
                        //      0   符合要求
                        //      1   不符合要求
                        nRet = app.CheckItemLibraryCode(itemdom,
                                    sessioninfo.LibraryCodeList,
                                    out strLibraryCode,
                                    out strError);
                        if (nRet == -1)
    ...
    out strError);
            ...
            out string[] values)
        {
            ...
            LibraryServerResult result = this.PrepareEnvironment(true, true); // false
            if (result.Value == -1)
                return result;
            try
            {
            ...
            string strNewPassword)
        {
            ...
            try
            {
                ...
                int nRet = app.ChangeUserPassword(
                    sessioninfo.LibraryCodeList,
                    strUserName,
                    strOldPassword,
                    strNewPassword,
        ...
        // 权限：暂时不需要任何权限
        public LibraryServerResult VerifyBarcode(string strBarcode)
        {
            LibraryServerResult result = this.PrepareEnvironment(false);
            if (result.Value == -1)
                return result;
            ...
            bool bLastWriteTime = false)
        {
            ...
            for (int i = 0; i < subs.Length; i++)
            {
                ...
                {
                    results.AddRange(GetFilenames(sub.FullName, bLastWriteTime));
                    continue;
                }
                ...
                if (strCategory == "cfgs")
                {
                    if (strName == "listFileNames")
                    {
                        ...
                        for (int i = 0; i < subs.Length; i++)
                        {
                            ...
                            filenames.AddRange(GetFilenames(sub.FullName));
                        }
                    ...
                    if (strName == "listFileNamesEx")
                    {
                        ...
                        {
                            DirectoryInfo sub = subs[i];
                            filenames.AddRange(GetFilenames(sub.FullName, true));
                        }
            ...
            try
            {
                ...
                // TODO: 读者身份是否还需要作更细致的限定?
                {
                        ...
                        out strLibraryCode) == true)
                    {
                        ...
                        {
                            result.Value = -1;
                            result.ErrorInfo = "读者身份不被允许用GetRes()来获得读者记录";
                            result.ErrorCode = ErrorCode.SystemError;
                            return result;
                        }
    ...
    strStyle);
        ...
        static int GetCommentParentPath(LibraryApplication app,
            XmlDocument comment_dom,
            ...
            out string strError)
        {
            ...
            string strRootID = DomUtil.GetElementText(comment_dom.DocumentElement,
                "parent");
