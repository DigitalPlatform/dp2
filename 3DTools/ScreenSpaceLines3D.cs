...
namespace _3DTools
{
    ...
    public class ScreenSpaceLines3D : ModelVisual3D
    {
        public ScreenSpaceLines3D()
        {
            ...
            _OnRenderIsregistered = true;
        }

        private bool _OnRenderIsregistered = false;


        protected override void OnVisualParentChanged(DependencyObject oldParent)
        {
            if (VisualTreeHelper.GetParent(this) == null && oldParent != null && _OnRenderIsregistered)
            {
                CompositionTarget.Rendering -= OnRender;
                _OnRenderIsregistered = false;
            }
            else if (VisualTreeHelper.GetParent(this) != null && oldParent == null && !_OnRenderIsregistered)
            {
                CompositionTarget.Rendering += OnRender;
                _OnRenderIsregistered = true;
            }
