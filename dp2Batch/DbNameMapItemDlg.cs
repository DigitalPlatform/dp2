...
namespace dp2Batch
{
	...
	public class DbNameMapItemDlg : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.button_directMap.Text = "直接对应(&D)";
		...
		private void button_explain_Click(object sender, System.EventArgs e)
		{
			...
			if (this.textBox_origin.Text == "{null}")
			{
				if (this.WriteMode == "skip")
					...
					strText = "将源数据文件中没有来源库信息的记录, 以 " +this.WriteMode + "方式写入目标 数据库 "+this.textBox_target.Text+" 中。";
				goto DONE;
			}
				...
				&& this.textBox_target.Text == "*")
			{
				if (this.WriteMode == "skip")
					...
					strText = "将源数据文件中有来源数据库信息的记录, 以 " +this.WriteMode+ " 方式写入服务器端同样数据库中。";
				goto DONE;
			}
				...
				&& this.textBox_target.Text != "*")
			{
				if (this.WriteMode == "skip")
					...
					strText = "将源数据文件中有来源数据库信息的记录, 不管其来源数据库名是什么, 都以 " +this.WriteMode+ " 方式写入服务器端数据库 "+this.textBox_target.Text+" 中。";
				goto DONE;
			}
				...
				&& this.textBox_target.Text == "*")
			{
				if (this.WriteMode == "skip")
					...
					strText = "将源数据文件中的记录, 如果其随文件记载的来源数据库名是 "+this.textBox_origin.Text+" ，则以 " +this.WriteMode+ " 方式写入服务器端同名数据库中。";
				goto DONE;
			}
			...
			if (this.WriteMode == "skip")
				...
				strText = "将源数据文件中的记录, 如果其随文件记载的来源数据库名是 "+this.textBox_origin.Text+" ，则以 " +this.WriteMode+ " 方式写入服务器端数据库 "+this.textBox_target.Text+" 中。";
