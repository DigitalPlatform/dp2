...
{
    public class MarcDiffBuilder : ISideBySideDiffBuilder
    {
        private readonly IDiffer differ;

        delegate void PieceBuilder(string oldText, string newText, List<DiffPiece> oldPieces, List<DiffPiece> newPieces);

        public static readonly char[] WordSeparaters = new[] { (char)31 };  // changed

        public MarcDiffBuilder(IDiffer differ)
        {
        ...
        private SideBySideDiffModel BuildLineDiff(string oldText, string newText)
        {
            ...
            var diffResult = differ.CreateLineDiffs(oldText, newText, false);   // changed
