    ...
    var menuAPI = $tree.jqTreeContextMenu($('#myMenu'), {
        "downloadExcel": function (node) {
            ...
            if (node.url != null) {
                ...
                // 打包下载
                // alert("请选择一个文件节点进行下载");
                $('#report_view').contents().find('html').html("<h1 style='text-align: center;'>打包下载 '" + node.id + "' 对应的 Excel 文件</h1>");
                $("#report_view").attr("src", "report.aspx?id=" + encodeURIComponent(node.id) + "&format=excel");
            }
        },
