    ...
    "<table class='messagelist_total'>" //  width='100%' cellspacing='1' cellpadding='4'
        ...
        void CreateBoxListControls(PlaceHolder boxlist)
        {
            ...
            for (int i = 0; i < this.MessageCenter.Boxes.Count; i++)
            {
                ...
                linkbutton = new LinkButton();
                linkbutton.ID = box.Name;
                string strCaption = this.MessageCenter.GetString(box.Type);   // 获得信箱对于当前语言的名字 2009/7/14 changed
                if (nCount != 0)
        ...
        void deleteallbutton_Click(object sender, EventArgs e)
        {
            ...
            for (; ; )
            {
                ...
                {
                    // text-level: 内部错误
                    this.SetDebugInfo("errorinfo", "删除全部消息时发生错误: " + strError);
                    return;
                }
        ...
        public void SetResultInfo()
        {
            ...
            if (this.ResultCount != 0)
            {
                ...
                // this.MessageCenter.GetString(this.CurrentBoxType) + " 内共有消息 " + Convert.ToString(this.ResultCount) + " 条, 分 " + Convert.ToString(this.PageCount) + " 页显示, 当前为第 " + Convert.ToString(nPageNo + 1) + "页。";
            }
            else
            {
                // text-level: 界面提示
