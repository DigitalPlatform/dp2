...
// 3.202 2026/3/20  改进 AddPinyin() 函数，允许通讯不良的时候能随时中断 API 调用，不冻结界面。
// 3.203 2026/3/20  将书目查询窗复制和移动书目记录的处理放在非界面线程中执行，避免界面冻结。
...
//                  导出读者详情功能遇到偶然的 SearchCharging() API 超时会抛出异常。为此增强了 ChargingHistoryLoader 类，增加了 Prompt 事件。这样遇到 SearchCharging() API 出错时会弹出一个对话框允许操作者选择重试。