...
namespace dp2Circulation
{
    ...
    public partial class ReaderSearchForm : SearchFormBase
    {
        ...
        private void toolStripButton_search_Click(object sender, EventArgs e)
        {
            ...
            stop.BeginLoop();
            */
            var looping = BeginLoop(this.DoStop, "正在检索 '" + this.textBox_queryWord.Text + "'...");
...
out strError);
        ...
        private void listView_records_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            bool bLooping = HasLooping();    // TODO: 需要重构
            {
            ...
            // 正在检索的时候，不允许进行批处理操作。因为stop.BeginLoop()嵌套后的Min Max Value之间的保存恢复问题还没有解决
            {
                ...
                subMenuItem = new MenuItem("超期通知 [" + this.listView_records.SelectedItems.Count.ToString() + "] (&N)");
                subMenuItem.Click += new System.EventHandler(this.menu_notifyPatron_Click);
                if (this.listView_records.SelectedItems.Count == 0
                    || this.InSearching == true)
                    subMenuItem.Enabled = false;
                subMenuItem.Tag = "notifyOverdue";
                menuItem.MenuItems.Add(subMenuItem);

                subMenuItem = new MenuItem("召回通知 [" + this.listView_records.SelectedItems.Count.ToString() + "] (&N)");
                subMenuItem.Click += new System.EventHandler(this.menu_notifyPatron_Click);
                if (this.listView_records.SelectedItems.Count == 0
                    || this.InSearching == true)
                    subMenuItem.Enabled = false;
                subMenuItem.Tag = "notifyRecall";
                menuItem.MenuItems.Add(subMenuItem);
...
MessageBoxDefaultButton.Button2);
            ...
            stop.BeginLoop();
            */
            var looping = BeginLoop(this.DoStop, "正在删除读者记录 ...", "halfstop");
    ...
    MessageBoxDefaultButton.Button1);
        ...
        void menu_verifyPatronRecord_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            stop.BeginLoop();
            */
            var looping = BeginLoop(this.DoStop, "正在校验读者记录 ...", "halfstop");
...
"<不使用>");
    ...
    "");
        ...
        void menu_moveRecords_Click(object sender, EventArgs e)
        {
            ...
            stop.BeginLoop();
            */
            var looping = BeginLoop(this.DoStop, "移动读者记录 ...", "halfstop");
...
MessageBoxDefaultButton.Button2);
        ...
        // 催还通知/召回通知
        void menu_notifyPatron_Click(object sender, EventArgs e)
        {
            string strError = "";
            ...
            string strAction = ((MenuItem)sender).Tag as string;

            string reason = "";
            if (strAction == "notifyRecall")
            {
                reason = InputDlg.GetInput(this,
    "请输入召回事由描述",
    "事由描述:",
    "毕业手续需要",
    this.Font);
            ...
            {
                dlg.Font = this.Font;
                dlg.TypeList = Program.MainForm.AppInfo.GetString("readersearchform", "messageTypeList", "mq");
                if (dlg.ShowDialog(this) != DialogResult.OK)
                    return;
                message_type_list = dlg.TypeList;
                Program.MainForm.AppInfo.SetString("readersearchform", "messageTypeList", dlg.TypeList);
            }

            string style = "";
            if (strAction == "notifyRecall")
                style = $"bodytypes:{message_type_list.Replace(",", "|")},reason:{StringUtil.EscapeString(reason, ":,")}";
            else
                style = $"bodytypes:{message_type_list.Replace(",", "|")}";

            Program.MainForm.OperHistory.AppendHtml("<div class='debug begin'>" + HttpUtility.HtmlEncode(DateTime.Now.ToLongTimeString()) + " 开始通知读者</div>");
            ...
            stop.BeginLoop();
            */
            var looping = BeginLoop(this.DoStop, "正在通知读者 ...", "halfstop");
...
out strError);
            ...
            finally
            {
                ...
                */
                ...
                Program.MainForm.OperHistory.AppendHtml("<div class='debug end'>" + HttpUtility.HtmlEncode(DateTime.Now.ToLongTimeString()) + " 结束通知读者</div>");
            }
...
out strError);
            ...
            out string strError)
        {
            ...
            LibraryChannel channel = this.GetChannel();
            var looping = BeginLoop(this.DoStop, "正在装入指纹数据 ...");
            try
            {
                for (; ; )
                {
            ...
            out string strError)
        {
            ...
            var looping = BeginLoop(this.DoStop, "正在获得读者库名字 ...");
            LibraryChannel channel = this.GetChannel();
            try
            {
                // 确保登录一次
...
dlg.UiState);
            ...
            out string strError)
        {
            ...
            stop.BeginLoop();
            */
            var looping = BeginLoop(this.DoStop, "导出读者借阅历史 ...", "halfstop");

            this.ShowMessage("正在导出读者借阅历史 ...");
            EnableControls(false);
            this.Invoke((Action)(() =>
            {
                this.listView_records.Enabled = false;
                ...
                if (nCount > 0)
                {
                    ...
                    _ = Task.Factory.StartNew(() =>
                    {
                        ...
                        */
                        ...
                        try
                        {
                            int nRet = form.ImportFromRecPathFile(fileName,
                                "",
                                out string error);
                            if (nRet == -1)
                                form.MessageBoxShow(error);
                        }
                        finally
                        {
                            form?.EnableControls(true);
            ...
            out string strError)
        {
            ...
            */
            ...
            stop.BeginLoop();
            */
            var looping = BeginLoop(this.DoStop, "正在处理读者记录 ...", "halfstop");
            ...
            out string strError)
        {
            ...
            LibraryChannel channel = this.GetChannel();
            var looping = BeginLoop(this.DoStop, "正在下载图片 ...");
            try
            {
...
dlg.UiState);
            ...
            XLWorkbook doc = null;
            var looping = BeginLoop(this.DoStop, "正在导出读者详情 ...");
            try
            {
