...
namespace dp2Circulation
{
    ...
    out strError*/);
        ...
        public new LibraryChannel Channel
        {
            get
            {
                throw new Exception("ManagerForm 的 Channel 成员已经废止");
            }
        }
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在删除数据库 " + strDatabaseNames + "...");
            _stop.BeginLoop();
    ...
    out string strError)
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在刷新数据库 " + strDatabaseNames + " 的定义...");
            _stop.BeginLoop();
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在初始化数据库 " + strDatabaseNames + "...");
            _stop.BeginLoop();
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在修改数据库 " + strDatabaseNames + "...");
            _stop.BeginLoop();
        ...
        REDO_LOGIN:
            ...
            try
            {
                ...
                //      0   登录未成功
                //      1   登录成功
                long lRet = channel.Login(login_dlg.UserName,
                    login_dlg.Password,
                    strParameters,
                    out strError);
                if (lRet == -1)
                ...
                if (lRet == 0)
                {
                    if (channel.ErrorCode == ErrorCode.TempCodeMismatch
                        || channel.ErrorCode == ErrorCode.RetryLogin)
                    {
                    ...
                    if (channel.ErrorCode == ErrorCode.NeedSmsLogin)
                    {
...
style_dlg.UiState);
    ...
    "settimeout:0:0:5");
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在获取全部 OPAC 数据库定义 ...");
            _stop.BeginLoop();
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在获取数据库 " + strDbName + " 的定义...");
            _stop.BeginLoop();
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在获取全部OPAC记录显示格式定义 ...");
            _stop.BeginLoop();
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在设置全部OPAC记录显示格式定义 ...");
            _stop.BeginLoop();
        ...
        out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在获取<locationTypes>配置 ...");
            _stop.BeginLoop();
            ...
            out string strError)
        {
            ...
            _stop.OnStop += new StopEventHandler(this.DoStop);
            _stop.Initial("正在设置<locationTypes>定义 ...");
            _stop.BeginLoop();
        ...
        MessageBoxDefaultButton.Button2);
            ...
            out strPureName);
                ...
                if (Global.IsGlobalUser(libraryCodeList) == false
                    && StringUtil.IsInList(strLibraryCode, libraryCodeList) == false)
                    bOwner = false;
            }
            ...
            dlg.IsGlobalUser = Global.IsGlobalUser(Program.MainForm._currentLibraryCodeList);
            dlg.CreateMode = false;
        ...
        private void toolStripButton_calendar_new_Click(object sender, EventArgs e)
        {
            ...
            dlg.OwnerLibraryCodes = GetOwnerLibraryCodes();
            dlg.CreateMode = true;
            dlg.CalendarName = Global.BuildCalendarName(Global.GetFirstLibraryCode(Program.MainForm._currentLibraryCodeList), "新建日历");    // 当前用户所在分馆
        ...
        private void tabControl_main_SelectedIndexChanged(object sender, EventArgs e)
        {
                ...
                && _kernelInitialized == false)
            {
                ...
                {
                    this.ShowMessage("内核管理功能要求当前连接的 dp2library 服务器版本为 2.117 或以上 (而现在是 " + Program.MainForm.ServerVersion + ")", "red", true);
                    return;
                }
