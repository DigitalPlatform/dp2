            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int i = 0; ; i++)
                    {
                        ...
                        _stop.SetMessage("正在装入路径 " + strOrderRecPath + " 对应的记录...");
    ...
    RemoveChangedChar(ListViewUtil.GetItemText(item, ORIGIN_COLUMN_FIXEDPRICE)));
            ...
            out string strError)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                _stop.SetMessage("正在输出事项 " + strSeller + " " + (i + 1).ToString());
...
string strFileName)
    ...
    out strError);
                ...
                foreach (DigitalPlatform.LibraryClient.localhost.Record searchresult in loader)
                {
                    ...
                    _stop.SetMessage("共命中 " + lHitCount.ToString() + " 条，已装入 " + (i + 1).ToString() + " 条");
                    i++;
                }
