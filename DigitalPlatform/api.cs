...
namespace DigitalPlatform
{
    ...
    UIntPtr wParam, IntPtr lParam);
        ...
        public static void GetEditSelection(TextBox edit,
            out int start_pos1,
            out int end_pos1,
            out int start_pos2,
            out int end_pos2)
        {
            var ptr1 = Marshal.AllocHGlobal(8);
            var ptr2 = Marshal.AllocHGlobal(8);
            try
            {
                int ret = (int)SendMessage(edit.Handle,
                    EM_GETSEL,
                    ptr1,
                    ptr2);

                start_pos1 = Marshal.ReadInt32(ptr1);
                end_pos1 = Marshal.ReadInt32(ptr2);

                // The return value is a zero-based value with the starting position of the selection in the LOWORD 
                // get the low-order word from sel
                start_pos2 = LoWord(ret);
                end_pos2 = HiWord(ret);
            }
            finally
            {
            ...
            // https://www.autohotkey.com/boards/viewtopic.php?t=4840
        }
