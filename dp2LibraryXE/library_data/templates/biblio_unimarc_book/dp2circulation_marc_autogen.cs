// dp2circulation 中文图书 MARC 编目自动创建数据C#脚本程序
// DetailHost版本
// 最后修改时间: 2014/11/4
...
// 3) 2009/8/8 增加维护856功能
// 4) 2011/1/10 AddPinyin()函数作了修改，所创建的$9子字段会紧贴着汉字子字段后面插入
// 5) 2011/6/22 增加调用单套卡片打印功能
// 6) 2011/8/17 采用新的(基类)加拼音函数
// 7) 2011/8/17 删除Main()函数，改用最新的动态菜单方式
// 8) 2011/8/22 修改AddAuthorNumber()函数，使之具有忽略第一指示符为'A'的7XX字段$a子字段的能力，并会跳过不包含汉字字符的著者字符串继续向后找
// 9) 2011/8/23 将函数名AddAuthorNumber()修改为AddGcatAuthorNumber()
// 10) 2011/8/23 增加AddSjhmAuthorNumber()。这只是适用于在905$e中加入著者号
// 11) 2011/8/29 Copy200gfTo7xxa()和Copy690aTo905d()函数修改，增加了对字段不存在时的判断和警告
// 12) 2013/9/13 加拼音能根据 MainForm 的 AutoSelPinyin 参数变化效果
// 13) 2014/11/4 从亚马逊加入封面图像功能
// 14) 2014/12/30 从亚马逊加入封面图片 856 字段功能，增加了根据 ISBN 决定访问亚马逊服务器地址的功能
// 15) 2015/12/2 注释掉和 905 字段有关的几个菜单项，以避免用户发生误会
...
public class MyHost : DetailHost
{
    ...
    public void CreateMenu(object sender, GenerateDataEventArgs e)
    {
        ...
        if (sender is MarcEditor || sender == null)
        {
            ...
            // 打印单套卡片
            actions.NewItem("打印单套卡片", "打印单套目录卡片", "PrintSingleCard", false);
    ...
    void PrintSingleCard()
    {
        ...
        if (nRet == -1)
        {
            if (string.IsNullOrEmpty(strError) == false)
                MessageBox.Show(this.DetailForm, "书目统计窗: " + strError);
        }
        else
            form.Close();
    }
