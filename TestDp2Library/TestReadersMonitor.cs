...
namespace TestDp2Library
{
    ...
    public class TestReadersMonitor
    {
        ...
        public void TestReadersMonitor_CheckNotifyPoint_1(
            string today,
            string borrowDate,
            string period,
            string notifyDef,
            ...
            int[] expect_indices)
        {
            if (expect_indices != null)
            {
                ...
                {
                    // 索引值必须小于检查点个数
                    var checkpoint_count = StringUtil.SplitList(notifyDef).Count;
                    Assert.True(expect_indices[i] >= 0 && expect_indices[i] < checkpoint_count);
                }
            }
        ...
        [Theory]
        // 有检查点在范围之外
        // -7day 这一时刻在 5 天范围之外，不会被返回 
        [InlineData("2021-10-26", "Tue, 26 Oct 2021 16:13:50 +0800",
            "5day", "-7day,-5day,-2day,-1day", 0, new int[] { 1 })]
        [InlineData("2021-10-31", "Tue, 26 Oct 2021 16:13:50 +0800",
            "5day", "-5day,6day", 0, new int[] { 0 })]

        public void TestReadersMonitor_CheckNotifyPoint_2(
    string today,
    string borrowDate,
    string period,
    string notifyDef,
