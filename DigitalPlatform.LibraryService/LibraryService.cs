...
out strError);
                ...
                if (LibraryApplication.IsObjectPath(strResPath))
                {
                    ...
                    //      1   允许读出
                    int nRet = IsRecordReadable(sessioninfo,
                        channel,
                        strRecordPath,
                        out strError);
                    if (nRet == -1)
...
out baOutputTimestamp);
                    ...
                    else
                    {
                        ...
                        if (LibraryApplication.IsObjectPath(strResPath))
                        {
                            ...
                            //      0   不在控制范围
                            //      1   在控制范围
                            int nRet = IsItemWriteable(sessioninfo,
                                channel,
                                strRecordPath,
                                out strError);
                            if (nRet == -1)
    ...
    strStyle);
        ...
        //      0   不允许读出。错误信息在 strError 中返回
        //      1   允许读出
        int IsRecordReadable(SessionInfo sessioninfo,
            RmsChannel channel,
            ...
            out string strError)
        {
            ...
            if (db_type == "reader")
            {
                ...
                {
                    if (app.IsCurrentChangeableReaderPath(strItemRecPath, sessioninfo.ExpandLibraryCodeList))
                        return 1;
