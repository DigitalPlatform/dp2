...
namespace dp2Library
{
    ...
    email* 
        ...
        // 2026/3/3 此前版本，本函数中的若干 id 被误改 user_id
...
out error);
        ...
        //      lCount  要获取的个数
        //      strBrowseInfoStyle  所返回的SearchResult中包含哪些信息。为逗号分隔的字符串列表值，取值可为 id/cols 之一。例如，"user_id,cols"表示同时获取id和浏览信息各列，而"user_id"表示仅取得id列。
    ...
    strDbName);
        ...
        static string GetModifiedStyle(string strBrowseInfoStyle)
        {
            ...
            string modified_style = strBrowseInfoStyle + ",id"; // 2026/3/3 此前版本这里被误改为 user_id
...
out timestamp);
                ...
                if (app.IsReaderDbName(strDbName))
                {
...

                    {
                        ...
                        {
                            strError = "读者记录 '" + strPath + "' 装入XMLDOM时出错: " + ex.Message;
                            goto ERROR1;
                        }

                        // 2023/1/16
                        // 过滤掉一些元素
                        app.FilterReaderRecord(readerdom, strLibraryCode);
                        strXml = readerdom.DocumentElement.OuterXml;
                    }

                    int nRet = app.BuildReaderResults(
    sessioninfo,
    null,
    strXml,   // strXml,
    LibraryApplication.GetReaderInfoLevel("getreaderinfo", sessioninfo.Rights),
    "xml",
    strLibraryCode,
    null,   // recpaths,
    null, // strMetadata,
...
out QueryResult[] results)
    ...
    current.Substring("!error:".Length));
                            else
                                FormatItem.SetFormat(items,
                                    i,
                                    formats[i],
            ...
            out string strError)
        {
            ...
            if (strRefID[0] == '@')
            {
                ...
                out strError);
                    ...
                    // 检查记录路径是不是 中文图书实体/1 这样的两段形态
                    if (app.IsDatabaseMetadataPath(sessioninfo, strIssueRecPath) == false)
                    {
                        strError = $"记录路径 '{strIssueRecPath}' 不允许使用元数据记录路径以外的形态";
                        goto ERROR1;
                    }
                ...
                */
                ...
                // TODO: 读者身份是否还需要作更细致的限定?
                {
                        ...
                        out strLibraryCode) == true)
                    {
                        ...
                        {
                            // strError = "不允许用 GetRes() 来获得读者记录。请改用 GetReaderInfo() API";
                            // goto ERROR1;
                            /*
                            if (StringUtil.IsInList("withresmetadata", strStyle))
                            {
                                strError = "获取读者记录时暂不支持 strStyle 包含 withresmetadata 用法";
                                goto ERROR1;
                            }
                            */
                            ...
                            /*
                            if (StringUtil.IsInList("prev", strStyle))
                                path += "$prev";
                            else if (StringUtil.IsInList("next", strStyle))
                                path += "$next";
                            */
...
out byte[] temp_timestamp);
                            ...
                            if (xml != null)
                            {
                                int nRet = ChunkMemory.ReturnFragment(xml,
    nStart,
    nLength,
    strStyle,
    out baContent,
...
out strError);
                ...
                // 1) 如果是读者身份试图获得读者记录下的对象，要想办法检查对象所从属的读者 XML 记录的 Barcode 等于 sessioninfo.Account.Barcode
                // 2) 读者身份只能获取属于自己的预约到书记录下属的对象
                if (LibraryApplication.IsObjectPath(strResPath))
                {
                    string strRecordPath = LibraryApplication.GetMetadataPath(strResPath);
                    ...
                    //      1   允许读出
                    int nRet = app.IsRecordReadable(sessioninfo,
                        channel,
                        strRecordPath,
                        out strError);
                    if (nRet == -1)
        ...
        static string GetResult(string[] results,
    int index)
        {
            if (index == -1)
                return null;
...
out strError);
                    ...
                    ref result);
                        ...
                        if (nRet == 1)
                        {
                            // 从此开始 baContent 里面是一条记录的全部内容
                            baContent = full_data;
                            string strBiblio = Encoding.UTF8.GetString(baContent);
                            string set_action = "change";
                            if (string.IsNullOrEmpty(strResPath)
                                || ResPath.IsAppendRecPath(strResPath)
...
out baOutputTimestamp);
                    ...
                    {
                        // 1) 对读者和工作人员身份写入册记录下的对象记录要做进一步权限判断
                        ...
                        if (LibraryApplication.IsObjectPath(strResPath))
                        {
                            ...
                            //      0   不在控制范围
                            //      1   在控制范围
                            int nRet = app.IsItemWriteable(sessioninfo,
                                channel,
                                strAction,
                                strRecordPath,
                                out strError);
                            ...
                            {
                                result.Value = -1;
                                result.ErrorInfo = $"判断资源 '{strResPath}' 可否被修改的过程出错: {strError}";
                                result.ErrorCode = ErrorCode.SystemError;
                                return result;
                            }
                            if (nRet == 0)
