...
注: 
冒号左侧是数据格式，右侧是修饰属性。如果存在修饰属性，则两侧中间的冒号不可省略，注意不要误用为 `|` 竖线字符。
竖线字符是用来分隔多个修饰属性的。
...
添加修饰属性`backup` 会影响到返回内容的详略程度。带 `backup` 时，返回完整信息；不带 `backup` 时，返回的信息会依据当前登录用户的权限而有所限制。

关于修饰属性 `backup` 影响返回内容详略的情形，举例如下：
假设一个账户的存取定义权限如
...
注: iso2709|backup 用法为何会报错？因为这里误用了竖线，backup 是修饰属性，它左侧应该使用冒号。
